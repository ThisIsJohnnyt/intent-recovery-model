"""Standalone assertion tests for prepare_phase2_contrastive_candidate_corpus.py --
...
`python test_prepare_phase2_contrastive_candidate_corpus.py`, matching this repo's
existing script-based tooling convention.

Covers every fail-closed condition listed in
prepare_phase2_contrastive_candidate_corpus.py's own module docstring,
including the authorized historical-to-composite delta, plus positive
checks against the real pinned files.
...
ChatGPT's corrective design (independently agreed by both): added
canonicalize_val_split_bytes coverage for every required case (LF
checkout passes; uniform CRLF checkout passes and normalizes to the same
LF output; LF and CRLF inputs produce identical canonical output; mixed
endings, a bare carriage return, content drift, whitespace drift,
blank-line drift, a missing terminal newline in either form, and a BOM
all fail closed); updated verify_val_byte_identical_to_existing's tests
for its new canonical-bytes-based signature; moved the "missing existing
val file" negative test to load_canonical_existing_val_bytes, which now
owns that check; added a live check that load_canonical_existing_val_bytes
succeeds against the real, unmodified repository file on this exact
Windows checkout, with no manual normalization.
...
corpus had the identical fragility, previously undetected -- renamed
canonicalize_val_split_bytes to canonicalize_pinned_lf_bytes (now shared
by both the val split and the parent corpus) throughout; moved the parent
fingerprint-drift negative test from verify_pinned_fingerprints (which no
longer checks the parent -- see load_canonical_parent_bytes) to its new
home; added load_canonical_parent_bytes coverage (uniform-CRLF checkout
loads and normalizes; missing file fails closed; fingerprint drift fails
closed); the live checks now exercise load_canonical_parent_bytes and
parse_jsonl_records_from_bytes against the real, unmodified repository
file, with no manual normalization, matching what the corrected main()
actually does.
"""
import copy
import hashlib
...
import prepare_phase2_contrastive_candidate_corpus as p
...
historical_for_delta = p.load_jsonl_records(p.HISTORICAL_PROPOSAL_PATH)
composite_for_delta = p.load_jsonl_records(p.PROPOSAL_PATH)

msg = expect_system_exit(p.verify_composite_construction, historical_for_delta, composite_for_delta)
check("exact reviewed 12-to-16 composite delta passes", msg is None, detail=str(msg))

changed_frozen = copy.deepcopy(composite_for_delta)
changed_frozen[0]["output"]["narrative"] += " unauthorized"
msg = expect_system_exit(p.verify_composite_construction, historical_for_delta, changed_frozen)
check("change to one of the ten frozen records fails closed", msg is not None)

restored_removed_bullet = copy.deepcopy(composite_for_delta)
restored_removed_bullet[8]["output"]["bullets"].append(p.AUTHORIZED_REMOVED_BULLETS[8][0])
msg = expect_system_exit(p.verify_composite_construction, historical_for_delta, restored_removed_bullet)
check("non-exact P2-009 bullet subset fails closed", msg is not None)

changed_remediation_action = copy.deepcopy(composite_for_delta)
changed_remediation_action[9]["output"]["action_items"].append("Unauthorized action.")
msg = expect_system_exit(p.verify_composite_construction, historical_for_delta, changed_remediation_action)
check("P2-010 non-bullet field change fails closed", msg is not None)

wrong_new_category = copy.deepcopy(composite_for_delta)
wrong_new_category[12]["category"] = "cross_field_completeness"
msg = expect_system_exit(p.verify_composite_construction, historical_for_delta, wrong_new_category)
check("AT-C1 category drift fails closed", msg is not None)

wrong_new_difficulty = copy.deepcopy(composite_for_delta)
wrong_new_difficulty[14]["difficulty"] = "hard"
msg = expect_system_exit(p.verify_composite_construction, historical_for_delta, wrong_new_difficulty)
check("AT-C3 difficulty drift fails closed", msg is not None)
...
# 2026-08-10: signature changed to compare against independently-verified
# canonical bytes (passed in), not a fresh re-read of EXISTING_R2_VAL_PATH --
# see canonicalize_pinned_lf_bytes / load_canonical_existing_val_bytes below
# for where the "missing file" and CRLF-checkout-tolerant cases now live.
canonical_val_for_test = b'{"prompt": "p1", "target": "t1"}\n{"prompt": "p2", "target": "t2"}\n'

result = None
try:
    result = p.verify_val_byte_identical_to_existing(canonical_val_for_test, canonical_val_for_test, 2)
except SystemExit as e:
    result = str(e)
check("byte-identical val split passes", isinstance(result, dict), detail=str(result))

msg = expect_system_exit(
    p.verify_val_byte_identical_to_existing, canonical_val_for_test + b"extra", canonical_val_for_test, 2
)
check("val split byte mismatch fails closed", msg is not None)

msg = expect_system_exit(
    p.verify_val_byte_identical_to_existing,
    canonical_val_for_test.replace(b"\n", b"\r\n"),
    canonical_val_for_test,
    2,
)
...
# ---------------------------------------------------------------------------
# canonicalize_pinned_lf_bytes / load_canonical_existing_val_bytes -- added
# 2026-08-10 per Claude's repository-integration finding and ChatGPT's
# corrective design (independently agreed by both, per the standing
# authorization protocol): a stock Windows checkout of the existing R2
# validation split silently converts its committed LF bytes to CRLF, which
# the old flat fingerprint check treated as drift even though `git status`
# itself reports the file clean. Covers every required case.
# ---------------------------------------------------------------------------

print("\n=== canonicalize_pinned_lf_bytes (shared: val split + parent corpus) ===")

canon = b'{"prompt": "p1", "target": "t1"}\n{"prompt": "p2", "target": "t2"}\n'
canon_fp = hashlib.sha256(canon).hexdigest()
crlf_of_canon = canon.replace(b"\n", b"\r\n")

result = None
try:
    result = p.canonicalize_pinned_lf_bytes(canon, canon_fp, "test file")
except SystemExit as e:
    result = str(e)
...
result = None
try:
    result = p.canonicalize_pinned_lf_bytes(crlf_of_canon, canon_fp, "test file")
except SystemExit as e:
    result = str(e)
check("uniform CRLF checkout passes and produces the same LF output", result == canon, detail=str(result))

lf_result = p.canonicalize_pinned_lf_bytes(canon, canon_fp, "test file")
crlf_result = p.canonicalize_pinned_lf_bytes(crlf_of_canon, canon_fp, "test file")
check("LF and CRLF inputs produce identical generated output", lf_result == crlf_result)

mixed_endings = b'{"prompt": "p1", "target": "t1"}\r\n{"prompt": "p2", "target": "t2"}\n'  # one CRLF, one bare LF
msg = expect_system_exit(p.canonicalize_pinned_lf_bytes, mixed_endings, canon_fp, "test file")
check("mixed line endings fail closed", msg is not None)

bare_cr = canon.replace(b'"t1"}\n', b'"t1"}\r', 1)  # lone CR, not CRLF
msg = expect_system_exit(p.canonicalize_pinned_lf_bytes, bare_cr, canon_fp, "test file")
check("bare carriage return fails closed", msg is not None)

content_drift_crlf = crlf_of_canon.replace(b"p1", b"p1-ALTERED", 1)
msg = expect_system_exit(p.canonicalize_pinned_lf_bytes, content_drift_crlf, canon_fp, "test file")
check("content drift (post-normalization hash mismatch) fails closed", msg is not None)

whitespace_drift_crlf = crlf_of_canon.replace(b'"target"', b'"target" ')
msg = expect_system_exit(p.canonicalize_pinned_lf_bytes, whitespace_drift_crlf, canon_fp, "test file")
check("whitespace drift fails closed", msg is not None)

blank_line_drift_crlf = crlf_of_canon.replace(b'"t1"}\r\n', b'"t1"}\r\n\r\n', 1)
msg = expect_system_exit(p.canonicalize_pinned_lf_bytes, blank_line_drift_crlf, canon_fp, "test file")
check("blank-line drift fails closed", msg is not None)

missing_terminal_newline_lf = canon[:-1]
msg = expect_system_exit(p.canonicalize_pinned_lf_bytes, missing_terminal_newline_lf, canon_fp, "test file")
check("missing terminal newline (LF form) fails closed", msg is not None)

missing_terminal_newline_crlf = crlf_of_canon[:-2]
msg = expect_system_exit(p.canonicalize_pinned_lf_bytes, missing_terminal_newline_crlf, canon_fp, "test file")
check("missing terminal newline (CRLF form) fails closed", msg is not None)

bom_prefixed = b"\xef\xbb\xbf" + canon
msg = expect_system_exit(p.canonicalize_pinned_lf_bytes, bom_prefixed, canon_fp, "test file")
check("UTF-8 BOM fails closed", msg is not None)


print("\n=== load_canonical_existing_val_bytes ===")

with tempfile.TemporaryDirectory() as tmp:
    fake_val_path = Path(tmp) / "val.jsonl"
    fake_val_path.write_bytes(crlf_of_canon)

    _orig_existing_val_path = p.EXISTING_R2_VAL_PATH
    _orig_expected_val_fp = p.EXPECTED_EXISTING_R2_VAL_FINGERPRINT
    p.EXISTING_R2_VAL_PATH = fake_val_path
    p.EXPECTED_EXISTING_R2_VAL_FINGERPRINT = canon_fp
    try:
        result = None
        try:
            result = p.load_canonical_existing_val_bytes()
        except SystemExit as e:
            result = str(e)
        check("uniform-CRLF working file loads and normalizes to canonical LF", result == canon, detail=str(result))

        p.EXISTING_R2_VAL_PATH = Path(tmp) / "does_not_exist.jsonl"
        msg = expect_system_exit(p.load_canonical_existing_val_bytes)
        check("missing existing val file fails closed", msg is not None)
    finally:
        p.EXISTING_R2_VAL_PATH = _orig_existing_val_path
        p.EXPECTED_EXISTING_R2_VAL_FINGERPRINT = _orig_expected_val_fp


# ---------------------------------------------------------------------------
# load_canonical_parent_bytes -- added 2026-08-10 (second fix): mirrors
# load_canonical_existing_val_bytes exactly, since both now share
# canonicalize_pinned_lf_bytes. The parent-fingerprint-drift negative test
# moved here from verify_pinned_fingerprints, which no longer checks the
# parent at all (see the NOTE inside that function).
# ---------------------------------------------------------------------------

print("\n=== load_canonical_parent_bytes ===")

with tempfile.TemporaryDirectory() as tmp:
    fake_parent_path = Path(tmp) / "parent.jsonl"
    fake_parent_path.write_bytes(crlf_of_canon)

    _orig_parent_path = p.PARENT_PATH
    _orig_expected_parent_fp = p.EXPECTED_PARENT_FINGERPRINT
    p.PARENT_PATH = fake_parent_path
    p.EXPECTED_PARENT_FINGERPRINT = canon_fp
    try:
        result = None
        try:
            result = p.load_canonical_parent_bytes()
        except SystemExit as e:
            result = str(e)
        check("uniform-CRLF parent checkout loads and normalizes to canonical LF", result == canon, detail=str(result))

        p.EXPECTED_PARENT_FINGERPRINT = "0" * 64
        msg = expect_system_exit(p.load_canonical_parent_bytes)
        check("parent fingerprint drift fails closed", msg is not None)
        p.EXPECTED_PARENT_FINGERPRINT = canon_fp

        p.PARENT_PATH = Path(tmp) / "does_not_exist.jsonl"
        msg = expect_system_exit(p.load_canonical_parent_bytes)
        check("missing parent file fails closed", msg is not None)
    finally:
        p.PARENT_PATH = _orig_parent_path
        p.EXPECTED_PARENT_FINGERPRINT = _orig_expected_parent_fp


# ---------------------------------------------------------------------------
# parse_jsonl_records_from_bytes -- added 2026-08-10 (second fix): parses
# directly from canonical bytes, not a second unverified path read.
# ---------------------------------------------------------------------------

print("\n=== parse_jsonl_records_from_bytes ===")

records_from_bytes = p.parse_jsonl_records_from_bytes(canon)
check(
    "parses the same records as the equivalent text file would",
    records_from_bytes == [{"prompt": "p1", "target": "t1"}, {"prompt": "p2", "target": "t2"}],
    detail=str(records_from_bytes),
)
check(
    "LF and CRLF-normalized-to-LF inputs parse identically",
    p.parse_jsonl_records_from_bytes(canon) == p.parse_jsonl_records_from_bytes(crlf_of_canon.replace(b"\r\n", b"\n")),
)


# ---------------------------------------------------------------------------
# verify_pinned_fingerprints -- negative test for a missing file, on top of
# the existing live "current files verify clean" check further down.
# Patches a path (not the file itself) so the real pinned files are never
# touched. The parent fingerprint-drift case now lives with
# load_canonical_parent_bytes above, since verify_pinned_fingerprints no
# longer checks the parent at all.
# ---------------------------------------------------------------------------
...
# Build a fake 82-slot candidate (66 parent stand-ins + our 2 test proposals,
...
msg = expect_system_exit(p.verify_pinned_fingerprints)
check(
    "real pinned fingerprints (historical proposal/composite proposal/manifest/both benchmarks) verify clean",
    msg is None,
    detail=str(msg),
)

# 2026-08-10: the real regression check for Claude's repository-integration
# finding -- this must succeed against the actual, unmodified file on this
# actual Windows checkout, with no manual normalization performed by the
# test (unlike the earlier fatal run before the fix, which required
# transiently rewriting the working-tree file and restoring it afterward).
real_canonical_val_bytes = None
try:
    real_canonical_val_bytes = p.load_canonical_existing_val_bytes()
except SystemExit as e:
    real_canonical_val_bytes = str(e)
check(
    "real existing R2 val split loads and canonicalizes cleanly on this checkout, no manual normalization",
    isinstance(real_canonical_val_bytes, bytes),
    detail=str(real_canonical_val_bytes) if not isinstance(real_canonical_val_bytes, bytes) else "",
)
check(
    "real canonical val bytes match the pinned fingerprint exactly",
    isinstance(real_canonical_val_bytes, bytes)
    and hashlib.sha256(real_canonical_val_bytes).hexdigest() == p.EXPECTED_EXISTING_R2_VAL_FINGERPRINT,
)

# 2026-08-10 (second fix): the real regression check for the parent-corpus
# finding -- must also succeed against the actual, unmodified file on this
# actual Windows checkout, with no manual normalization, exercising the
# exact code path main() now uses (load, then parse from those same bytes).
real_canonical_parent_bytes = None
try:
    real_canonical_parent_bytes = p.load_canonical_parent_bytes()
except SystemExit as e:
    real_canonical_parent_bytes = str(e)
check(
    "real R2 parent corpus loads and canonicalizes cleanly on this checkout, no manual normalization",
    isinstance(real_canonical_parent_bytes, bytes),
    detail=str(real_canonical_parent_bytes) if not isinstance(real_canonical_parent_bytes, bytes) else "",
)
check(
    "real canonical parent bytes match the pinned fingerprint exactly",
    isinstance(real_canonical_parent_bytes, bytes)
    and hashlib.sha256(real_canonical_parent_bytes).hexdigest() == p.EXPECTED_PARENT_FINGERPRINT,
)

real_parent_records = (
    p.parse_jsonl_records_from_bytes(real_canonical_parent_bytes)
    if isinstance(real_canonical_parent_bytes, bytes)
    else []
)
check("real parent corpus has exactly 66 records", len(real_parent_records) == p.EXPECTED_PARENT_COUNT)

historical_proposal_records = p.load_jsonl_records(p.HISTORICAL_PROPOSAL_PATH)
check(
    "historical proposal has exactly 12 records",
    len(historical_proposal_records) == p.EXPECTED_HISTORICAL_PROPOSAL_COUNT,
)

real_proposal_records = p.load_jsonl_records(p.PROPOSAL_PATH)
check("real composite proposal has exactly 16 records", len(real_proposal_records) == p.EXPECTED_PROPOSAL_COUNT)
...
msg = expect_system_exit(p.verify_composite_construction, historical_proposal_records, real_proposal_records)
check("real composite matches the exact authorized historical delta", msg is None, detail=str(msg))
