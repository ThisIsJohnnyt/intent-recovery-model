"""Disabled-by-default Gate 5 V7 paid-pilot engine.

This module exposes only local verification at its command line.  Any future
real use must enter through the dedicated V7 campaign runner, which supplies
...
SCHEDULE_PATH = PACKAGE / "schedule_v7.json"
V7_RECONCILIATION_STOP = 3_750_000
V7_PILOT_CEILING = 5_000_000
...
def load_schedule() -> list[dict[str, Any]]:
    try:
        ...
        raise Gate5PilotStop("v7_schedule_unavailable") from exc
    ...
    if (
        ...
        or value.get("artifact") != "gemini_generator_schedule_v7"
        or value.get("algorithm") != historical.get("algorithm")
        or value.get("slot_count") != 24
        or value.get("model_counts") != historical.get("model_counts")
        or value.get("manifest_sha256") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload))
        or not isinstance(slots, list)
        or len(slots) != 24
        or [item.get("slot") for item in slots] != list(range(1, 25))
        or any(item.get("prompt_collision_preflight", {}).get("fatal") is not False for item in slots)
    ):
        raise Gate5PilotStop("v7_schedule_unavailable")
    system = v7_prompt.load_v7_system_instruction()
    references = gate2.build_quarantine()[1]
    for item, old in zip(slots, historical.get("slots", [])):
        if {key: item.get(key) for key in ("slot", "model", "mechanism_id")} != {key: old.get(key) for key in ("slot", "model", "mechanism_id")}:
            raise Gate5PilotStop("v7_schedule_historical_fallback")
        ...
        if item.get("prompt_hash") != expected_prompt_hash or item.get("prompt_collision_preflight") != expected_preflight or item.get("prompt_hash") == old.get("prompt_hash"):
            raise Gate5PilotStop("v7_schedule_historical_fallback")
    first_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(v7_prompt.build_request(slots[0])))
    if first_hash != v7_prompt.EXPECTED_LIVE_REQUEST_SHA256_V7:
        raise Gate5PilotStop("v7_request_pin_mismatch")
    return slots
...
def reservation_fits_reconciliation_stop(fresh_run_cumulative: int, next_reservation: int, historical_total: int = PRIOR_PILOT_BOOKED_COST) -> bool:
    if type(next_reservation) is not int or next_reservation < 0:
        raise Gate5PilotStop("pilot_cost_invalid")
    return aggregate_pilot_cost(fresh_run_cumulative + next_reservation, historical_total) <= V7_RECONCILIATION_STOP
...
def verify_local_build() -> dict[str, Any]:
    ...
    v7_slots = load_schedule()
    v7_hashes = [gate2.sha256_bytes(gate2.canonical_json_bytes(v7_prompt.build_request(slot))) for slot in v7_slots]
    ...
    return {
        "artifact": "gemini_generator_gate5_paid_pilot_v7_engine_verify_only",
        ...
        **third_evidence,
        ...
        "v7_schedule_sha256": canonical_hash(SCHEDULE_PATH),
        "v7_slot_one_request_sha256": v7_hashes[0],
        "v7_request_count": len(v7_hashes),
        "v7_unique_request_count": len(set(v7_hashes)),
        "network_used": False,
...
def prepare_output_directory(root: Path, contract_hash: str, provider_schema_hash: str, rate_hash: str, attestation_hash: str, success_receipt_row_hash: str, flash_lite_success_receipt_row_hash: str, fresh_evidence: dict[str, Any], third_evidence: dict[str, Any], historical_context: dict[str, Any] | None = None) -> dict[str, Path]:
    ...
    reservation = {
        ...
        "live_validated_request_envelope_sha256": execution_gate.EXPECTED_LIVE_REQUEST_SHA256,
        "v7_slot_one_request_sha256": v7_prompt.EXPECTED_LIVE_REQUEST_SHA256_V7,
        "v7_schedule_sha256": canonical_hash(SCHEDULE_PATH),
        ...
        **third_evidence,
        ...
        "slot_count": 24,
        "pilot_ceiling_usd_millionths": V7_PILOT_CEILING,
        "reconciliation_stop_usd_millionths": V7_RECONCILIATION_STOP,
...
def execute_pilot(
    ...
    try:
        for sequence, slot in enumerate(slots, 1):
            ...
            cumulative += actual
            if aggregate_pilot_cost(cumulative, historical_context["historical_pilot_actual_usd_millionths"]) > V7_PILOT_CEILING:
                disposition = "rejected"
...
def main() -> int:
    ...
    if not args.verify_only:
        parser.error("--verify-only is required; V7 execution is available only through the campaign runner")
