"""Disabled-by-default Gate 5 V10 full-schedule continue-past-schema_invalid engine.

This module exposes only local verification at its command line.  Any future
real use must enter through the dedicated V10 campaign runner, which supplies
...
V10 reuses V9's (and, through it, V8's) schedule, prompt builder, historical
legacy-tower verification, and per-row builders unchanged (imported directly,
not duplicated).  The only behavioral change from V9: a genuine `schema_invalid`
outcome (one where a schema-conformance diagnostic row was successfully built)
now continues the run to the next card instead of stopping it, matching
Johnny's explicit review-and-approve decision after V9's real run showed
`schema_invalid` cutting the campaign short at 2/22 cards.  Nothing else about
what stays hard-terminal changes: `secret_exposure`, either
`*_diagnostic_withheld` reason, every cost-tier and structural/integrity stop
remain exactly as hard-terminal as they were in V9 -- confirmed explicitly with
Johnny rather than silently carried forward or silently expanded further.
"""
...
import gate5_paid_pilot_v9_engine as v9
...
v8 = v9.v8
# V10 reuses V9's (and, through it, V8's) own schedule/prompt/legacy-tower
# artifacts unchanged; it does not get its own schedule file. All of these are
# re-derived and pinned at load time, not trusted from this constant alone.
EXPECTED_V8_SCHEDULE_SHA256 = v9.EXPECTED_V8_SCHEDULE_SHA256
EXPECTED_V10_FIRST_REQUEST_SHA256 = v9.EXPECTED_V9_FIRST_REQUEST_SHA256
# V8/V9 never pinned gate5_schema_conformance_evidence.py's own file hash in
# the pre-flight the way they always did for gate5_output_collision_evidence.py
# (see v8.OUTPUT_COLLISION_EVIDENCE_PATH/EXPECTED_OUTPUT_COLLISION_EVIDENCE_SHA256
# below) - a real asymmetry, harmless while schema_invalid was hard-terminal
# everywhere, worth closing now that V10 depends on this module's multi-row
# correctness for real. Added only here, in V10's own execute_pilot(); V8/V9
# are untouched.
SCHEMA_CONFORMANCE_EVIDENCE_PATH = PACKAGE / "gate5_schema_conformance_evidence.py"
EXPECTED_SCHEMA_CONFORMANCE_EVIDENCE_SHA256 = "7e3477540422d6de6c5bafc713f10b210274537e1d1a10455a8720f36b2579e5"
V10_RECONCILIATION_STOP = 3_750_000
V10_PILOT_CEILING = 5_000_000
MAX_CARD_ATTEMPTS = 5
# Every other rejection reason stays hard-terminal for the whole run. V10 adds
# exactly one exact-match case (schema_invalid) to V9's suffix-based
# collision-family set -- confirmed explicitly with Johnny, not assumed.
CONTINUE_PAST_REASON_SUFFIXES = v9.CONTINUE_PAST_REASON_SUFFIXES

Gate5PilotStop = v9.Gate5PilotStop
ProviderResponse = v9.ProviderResponse
HTTPSPilotTransport = v9.HTTPSPilotTransport
canonical_hash = v9.canonical_hash
utc_now = v9.utc_now
validate_provider_response = v9.validate_provider_response
load_execution_day_rates = v9.load_execution_day_rates
legacy_historical_components = v9.legacy_historical_components
validate_historical_components = v9.validate_historical_components
_historical_m01_slots = v9._historical_m01_slots
_prompt_references = v9._prompt_references
_legacy_slot_view = v9.v8._legacy_slot_view
_new_file = v9._new_file
_append = v9._append
verify_successful_diagnostic_evidence = v9.verify_successful_diagnostic_evidence
verify_successful_flash_lite_evidence = v9.verify_successful_flash_lite_evidence
verify_fresh_attempt_evidence = v9.verify_fresh_attempt_evidence
verify_third_attempt_evidence = v9.verify_third_attempt_evidence
# Pure row builders: unchanged from V9, reused directly rather than
# re-implemented (they carry no continue-past logic of their own).
_receipt = v9._receipt
_cost_row = v9._cost_row
_rejection = v9._rejection
_candidate_row = v9._candidate_row


def load_schedule() -> list[dict[str, Any]]:
    """Re-derive and pin the exact same 22-slot corrected-V8 schedule, unchanged."""
    slots = v9.load_schedule()
    if canonical_hash(v8.SCHEDULE_PATH) != EXPECTED_V8_SCHEDULE_SHA256:
        raise Gate5PilotStop("v10_schedule_source_mismatch")
    first_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(v8.v7_prompt.build_request(slots[0])))
    if first_hash != EXPECTED_V10_FIRST_REQUEST_SHA256:
        raise Gate5PilotStop("v10_request_pin_mismatch")
    return slots
...
def reservation_fits_reconciliation_stop(fresh_run_cumulative: int, next_reservation: int, historical_total: int) -> bool:
    if type(next_reservation) is not int or next_reservation < 0:
        raise Gate5PilotStop("pilot_cost_invalid")
    return aggregate_pilot_cost(fresh_run_cumulative + next_reservation, historical_total) <= V10_RECONCILIATION_STOP
...
def prepare_output_directory(root: Path, contract_hash: str, provider_schema_hash: str, rate_hash: str, attestation_hash: str, success_receipt_row_hash: str, flash_lite_success_receipt_row_hash: str, fresh_evidence: dict[str, Any], third_evidence: dict[str, Any], historical_total: int) -> dict[str, Path]:
    ...
    reservation = {
        "artifact": "gemini_generator_gate5_paid_pilot_v10_reservation",
        "created_utc": utc_now(),
        "contract_sha256": contract_hash,
        "provider_schema_sha256": provider_schema_hash,
        "v10_first_request_sha256": EXPECTED_V10_FIRST_REQUEST_SHA256,
        "v10_reused_v8_schedule_sha256": EXPECTED_V8_SCHEDULE_SHA256,
        ...
        **third_evidence,
        ...
        "pilot_ceiling_usd_millionths": V10_PILOT_CEILING,
        "reconciliation_stop_usd_millionths": V10_RECONCILIATION_STOP,
...
def _outcome_continues_past(stop_reason: str) -> bool:
    # V10's one behavioral change from V9: a genuine schema_invalid (schema
    # diagnostic successfully built) now continues the run to the next card,
    # per Johnny's explicit review-and-approve decision after V9's real run.
    # schema_conformance_diagnostic_withheld -- where the diagnostic itself
    # could not be built -- deliberately stays OUT of this set: an
    # evidence-integrity failure is not the same kind of thing as a
    # successfully-diagnosed content-quality miss, mirroring why
    # output_collision_diagnostic_withheld was never continue-past-eligible
    # in V9 either.
    if stop_reason in ("unexpected_http_status_retries_exhausted", "schema_invalid"):
        return True
...
def execute_pilot(
    ...
    whole run immediately on any other exceptional outcome."""
    try:
        ...
        # A real list, not a generator -- see V9's identical comment; the same
        # multi-diagnostic-per-run generator-exhaustion class of bug applies
        # here unchanged, since V10 can accumulate collision AND schema
        # diagnostics across many cards in a single run.
    ...
    try:
        for card_index, slot in enumerate(slots, 1):
            ...
            for card_attempt_number in range(1, MAX_CARD_ATTEMPTS + 1):
                ...
                cumulative += actual
                if aggregate_pilot_cost(cumulative, historical_total) > V10_PILOT_CEILING:
                    disposition = "rejected"
    ...
    summary = {
        "artifact": "gemini_generator_gate5_paid_pilot_v10_summary",
...
def verify_local_build() -> dict[str, Any]:
    ...
    return {
        "artifact": "gemini_generator_gate5_paid_pilot_v10_engine_verify_only",
        ...
        **third_evidence,
        ...
        "v10_reused_v8_schedule_sha256": canonical_hash(v8.SCHEDULE_PATH),
        "v10_first_request_sha256": hashes[0],
        "v10_request_count": len(hashes),
        "v10_unique_request_count": len(set(hashes)),
        "max_card_attempts": MAX_CARD_ATTEMPTS,
        "network_used": False,
...
def main() -> int:
    ...
    if not args.verify_only:
        parser.error("--verify-only is required; V10 execution is available only through the campaign runner")
