...
import gate2
import gate5_paid_pilot_retry_campaign_v7_gate as gate
import gate5_paid_pilot_retry_campaign_v7_runner as runner
...
import gate5_v7_narrative_idiom_prompt as v7_prompt
...
class Gate5PaidPilotRetryCampaignV7Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.valid = {
    ...
    def attestation_stub(self, root: Path, rate: Path, execution_date: str = "2026-08-17") -> tuple[Path, dict]:
        value = gate2.load_json(PACKAGE / "gate5_paid_pilot_retry_campaign_v7_attestation_template.json")
        value.update({**gate.EXPECTED_BUILD, **gate.EXPECTED_V6, **gate.current_build_hashes()})
    ...
    def fake_output(self, output: Path, components: list[dict], outcome: str) -> dict:
        ...
        request_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(pilot.v7_prompt.build_request(slot))); raw_hash = "b" * 64
        ...
        (output / "pilot_reservation.json").write_bytes(gate2.canonical_json_bytes({"artifact": "synthetic", "v7_schedule_sha256": runner.canonical_hash(pilot.SCHEDULE_PATH), "v7_slot_one_request_sha256": gate.EXPECTED_BUILD["v7_slot_one_request_sha256"]}))
    ...
    def test_real_paid_engine_persists_eligible_reasons_and_preserves_core_on_io_failure(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            self.assertEqual(receipt["request_hash"], v7_prompt.EXPECTED_LIVE_REQUEST_SHA256_V7)
            self.assertEqual(receipt["prompt_hash"], pilot.load_schedule()[0]["prompt_hash"])
            reservation = gate2.load_json(output / "pilot_reservation.json")
            self.assertEqual(reservation["v7_schedule_sha256"], pilot.canonical_hash(pilot.SCHEDULE_PATH))
            self.assertEqual(reservation["v7_slot_one_request_sha256"], v7_prompt.EXPECTED_LIVE_REQUEST_SHA256_V7)
    ...
    def test_v6_is_rederived_and_immutable(self) -> None:
        before = {relative: runner.canonical_hash(PACKAGE / relative) for relative in runner.V6_TOP_HASHES}
        context = runner.verify_initial_evidence(); self.assertEqual(context["historical_pilot_actual_usd_millionths"], 170_880); self.assertEqual(context["historical_pilot_component_count"], 18); self.assertEqual(context["historical_pilot_components_sha256"], gate.INITIAL_COMPONENT_MANIFEST_SHA256)
        self.assertEqual(before, {relative: runner.canonical_hash(PACKAGE / relative) for relative in runner.V6_TOP_HASHES})

    def test_v7_schedule_and_prompt_fallbacks_are_rejected(self) -> None:
        historical = gate2.load_json(PACKAGE / "schedule.json")
        revised = gate2.load_json(pilot.SCHEDULE_PATH)
        slots = pilot.load_schedule()
        self.assertEqual(len(slots), 24)
        for old, new in zip(historical["slots"], revised["slots"]):
            self.assertEqual({key: old[key] for key in ("slot", "model", "mechanism_id")}, {key: new[key] for key in ("slot", "model", "mechanism_id")})
            self.assertNotEqual(old["prompt_hash"], new["prompt_hash"])
            self.assertFalse(new["prompt_collision_preflight"]["fatal"])
        system = v7_prompt.load_v7_system_instruction()
        first_prompt = pilot._prompt_references(slots)[0][1]
        self.assertTrue(first_prompt.startswith(system + "\n"))
        self.assertFalse(first_prompt.startswith(v7_prompt.HISTORICAL_SYSTEM_PATH.read_text(encoding="utf-8") + "\n"))
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            root = Path(temp)
            fallback = copy.deepcopy(revised)
            fallback["slots"][0]["prompt_hash"] = historical["slots"][0]["prompt_hash"]
            payload = {key: item for key, item in fallback.items() if key != "manifest_sha256"}
            fallback["manifest_sha256"] = gate2.sha256_bytes(gate2.canonical_json_bytes(payload))
            path = root / "fallback-schedule.json"
            path.write_bytes(gate2.canonical_json_bytes(fallback))
            with patch.object(pilot, "SCHEDULE_PATH", path), self.assertRaisesRegex(pilot.Gate5PilotStop, "v7_schedule_historical_fallback"):
                pilot.load_schedule()
            with patch.object(v7_prompt, "V7_SYSTEM_PATH", v7_prompt.HISTORICAL_SYSTEM_PATH), self.assertRaisesRegex(v7_prompt.Gate5V7PromptError, "V7 system instruction drifted"):
                v7_prompt.load_v7_system_instruction()

    def test_private_diagnostic_campaign_root_is_exactly_scoped(self) -> None:
        runner._validate_campaign_root(PACKAGE / "gate5_paid_pilot_retry_campaign_v7_2026-08-17", "2026-08-17")
        for path, execution_date in ((PACKAGE / "campaign", "2026-08-17"), (PACKAGE / "gate5_paid_pilot_retry_campaign_v7_2026-08-16", "2026-08-17"), (PACKAGE.parent / "gate5_paid_pilot_retry_campaign_v7_2026-08-17", "2026-08-17")):
            with self.subTest(path=path), self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV7Stop, "campaign_directory_invalid"):
                runner._validate_campaign_root(path, execution_date)

    def test_v7_engine_has_no_standalone_execution_or_credential_entrypoint(self) -> None:
        source = (PACKAGE / "gate5_paid_pilot_v7_engine.py").read_text(encoding="utf-8")
        self.assertNotIn("--execute-pilot", source)
        self.assertNotIn("load_windows_generic_credential", source)
        self.assertNotIn("import gate4_connectivity_runner", source)

    def test_verify_only_and_attestation_remain_execution_gated(self) -> None:
        result = runner.verify_only(); self.assertFalse(result["network_used"] or result["credential_read"] or result["file_output_created"]); self.assertEqual(result["maximum_campaign_attempts"], 15); self.assertEqual(result["worst_case_aggregate_usd_millionths"], 3_230_880)
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            root = Path(temp); rates = self.rates(root); attestation, _ = self.attestation_stub(root, rates); value = gate2.load_json(attestation); value["v7_campaign_execution_authorized_by_johnny"] = False; attestation.write_bytes(gate2.canonical_json_bytes(value))
            with self.assertRaisesRegex(gate.Gate5PaidPilotRetryCampaignV7AttestationError, "unconfirmed"): gate.validate_attestation(attestation)
    ...
    def test_only_whitelist_pauses_and_schema_requires_diagnostic(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with self.assertRaises(runner.Gate5PaidPilotRetryCampaignV7Stop): runner._validate_attempt_output(root / "schema_invalid", components)
    ...
    def test_same_day_review_is_one_use_and_no_credential(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with patch.object(runner, "local_today", return_value="2026-08-17"):
                ...
                with self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV7Stop, "pause_review_invalid"): runner.confirm_pause_review(review, attestation, campaign)
    ...
    def test_next_day_review_accepts_equal_rates_and_rejects_drift(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with patch.object(runner, "local_today", return_value="2026-08-17"), self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV7Stop, "pause_review_invalid"): runner.confirm_pause_review(review, attestation, campaign, fresh)
    ...
    def test_next_day_review_left_until_day_n_plus_2_fails_at_reservation(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with patch.object(runner, "local_today", return_value="2026-08-18"), self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV7Stop, "execution_day_rate_snapshot_mismatch"): runner.reserve_attempt(campaign, runner.canonical_hash(attestation), fresh)
    ...
    def test_third_attempt_is_terminal_and_money_is_monotonic(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            self.assertEqual(completion["campaign_state_after"], "attempt_cap_reached"); final = pilot.validate_historical_components([*components, completion["component"]]); self.assertEqual(final["historical_pilot_actual_usd_millionths"], 170_880 + 14 * 204_000 + 10_680); self.assertLessEqual(final["historical_pilot_actual_usd_millionths"], 3_230_880)

    def test_old_v6_attestation_cannot_pass_v7_gate(self) -> None:
        with self.assertRaises(gate.Gate5PaidPilotRetryCampaignV7AttestationError):
            gate.validate_attestation(runner.V6_ATTESTATION)
    ...
    def test_credential_failure_is_zero_cost_evidenced_and_terminal(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            rows, components = runner.load_and_verify_campaign(campaign, runner.canonical_hash(attestation)); self.assertEqual(rows[-1]["campaign_state"], "stopped_nonretryable_outcome"); self.assertEqual(pilot.validate_historical_components(components)["historical_pilot_actual_usd_millionths"], 170_880)
    ...
    def test_review_field_tampering_fails_without_state_mutation(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with patch.object(runner, "local_today", return_value="2026-08-17"), self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV7Stop, "pause_review_invalid"): runner.confirm_pause_review(review, attestation, campaign)
    ...
    def test_two_pause_cycles_consume_attempts_and_preserve_live_cost(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            rows, final_components = runner.load_and_verify_campaign(campaign, digest); self.assertEqual(rows[-1]["attempts_reserved"], 2); self.assertEqual(rows[-1]["campaign_state"], "active_after_review"); self.assertEqual(pilot.validate_historical_components(final_components)["historical_pilot_actual_usd_millionths"], 170_880 + 2 * 10_680)
    ...
    def test_recovery_uses_existing_evidence_and_never_resends(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV7Stop, "no_incomplete_attempt_to_recover"): runner.recover_incomplete_attempt(attestation, campaign)
    ...
    def test_concurrent_attempt_reservation_has_one_winner(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            self.assertEqual(sum(isinstance(item, runner.Gate5PaidPilotRetryCampaignV7Stop) for item in outcomes), 2)
