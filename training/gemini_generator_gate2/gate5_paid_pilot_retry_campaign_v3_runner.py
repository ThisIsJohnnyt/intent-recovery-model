"""Disabled-by-default bounded manual campaign v3 for full Gate 5 pilot attempts."""
...
import gate5_paid_pilot_retry_campaign_v3_gate as campaign_gate
import gate5_paid_pilot_retry_campaign_v2_runner as v2
import gate5_paid_pilot_runner as pilot

PACKAGE = Path(__file__).resolve().parent
PROPOSAL_PATH = PACKAGE / "gate5_paid_pilot_retry_campaign_v3_output_collision_evidence_proposal.md"
REFERENCE_RATE_PATH = PACKAGE / "gate5_execution_day_rate_snapshot_2026-08-16.json"
V2_DIRECTORY = PACKAGE / "gate5_paid_pilot_retry_campaign_v2_2026-08-16"
V2_ATTESTATION = PACKAGE / "gate5_paid_pilot_retry_campaign_v2_attestation_2026-08-16.json"
BUILD_PATHS = {
    "output_collision_evidence_proposal_sha256": PACKAGE / "gate5_output_collision_evidence_proposal.md",
    "structured_collision_gate2_sha256": PACKAGE / "gate2.py",
    "output_collision_evidence_module_sha256": PACKAGE / "gate5_output_collision_evidence.py",
    "output_collision_integrated_paid_runner_sha256": PACKAGE / "gate5_paid_pilot_runner.py",
    "output_collision_focused_tests_sha256": PACKAGE / "test_gate5_output_collision_evidence.py",
    "v3_campaign_runner_sha256": PACKAGE / "gate5_paid_pilot_retry_campaign_v3_runner.py",
    "v3_campaign_tests_sha256": PACKAGE / "test_gate5_paid_pilot_retry_campaign_v3.py",
}
...
class Gate5PaidPilotRetryCampaignV3Stop(RuntimeError):
...
def canonical_hash(path: Path) -> str:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV3Stop("canonical_input_invalid") from exc
...
def _new_file(path: Path, data: bytes) -> None:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV3Stop("exclusive_output_unavailable") from exc
...
def _append(path: Path, row: dict[str, Any]) -> None:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV3Stop("campaign_state_append_failed") from exc
...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    except (OSError, UnicodeDecodeError, json.JSONDecodeError, campaign_gate.Gate5PaidPilotRetryCampaignV3AttestationError) as exc:
        raise Gate5PaidPilotRetryCampaignV3Stop("campaign_state_invalid") from exc


def initial_components() -> list[dict[str, Any]]:
    attestation_hash = canonical_hash(V2_ATTESTATION)
    rows, components = v2.load_and_verify_campaign(V2_DIRECTORY, attestation_hash)
    ...
    if rows[-1].get("campaign_state") != "stopped_nonretryable_outcome" or rows[-1].get("attempts_reserved") != 1:
        raise Gate5PaidPilotRetryCampaignV3Stop("v2_terminal_state_invalid")
    if context != {"historical_pilot_component_count": 6, "historical_pilot_components_sha256": campaign_gate.INITIAL_COMPONENT_MANIFEST_SHA256, "historical_pilot_actual_usd_millionths": 53_400}:
        raise Gate5PaidPilotRetryCampaignV3Stop("initial_component_manifest_invalid")
    if components[-1].get("booked_cost_usd_millionths") != 10_680 or components[-1].get("terminal_disposition") != "proposed_output:output.narrative:protected_collision":
        raise Gate5PaidPilotRetryCampaignV3Stop("v2_terminal_component_invalid")
    return components
...
def _load_rows(path: Path) -> list[dict[str, Any]]:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV3Stop("historical_pilot_evidence_invalid") from exc
...
def _verify_summary(directory: Path, expected: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], bytes]:
    ...
    if summary.get("summary_sha256") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)):
        raise Gate5PaidPilotRetryCampaignV3Stop("historical_pilot_evidence_invalid")
...
def verify_initial_evidence() -> dict[str, Any]:
    paths = {
        "v2_final_attestation_sha256": V2_ATTESTATION,
        "v2_terminal_campaign_state_sha256": V2_DIRECTORY / v2.LEDGER_NAME,
        "v2_attempt_1_lock_sha256": V2_DIRECTORY / "attempt_001_lock.json",
        "v2_attempt_1_completion_sha256": V2_DIRECTORY / "attempt_001_completion.json",
        "v2_attempt_1_reservation_sha256": V2_DIRECTORY / "attempt_001_pilot_output" / "pilot_reservation.json",
        "v2_attempt_1_receipts_sha256": V2_DIRECTORY / "attempt_001_pilot_output" / "request_receipts.jsonl",
        "v2_attempt_1_rejection_sha256": V2_DIRECTORY / "attempt_001_pilot_output" / "rejection_ledger.jsonl",
        "v2_attempt_1_cost_sha256": V2_DIRECTORY / "attempt_001_pilot_output" / "cost_ledger.jsonl",
        "v2_attempt_1_summary_sha256": V2_DIRECTORY / "attempt_001_pilot_output" / "run_summary.json",
    }
    for field, path in paths.items():
        if canonical_hash(path) != campaign_gate.EXPECTED_V3[field]:
            raise Gate5PaidPilotRetryCampaignV3Stop("v2_terminal_evidence_invalid")
    quarantine = V2_DIRECTORY / "attempt_001_pilot_output" / "candidate_quarantine.jsonl"
    if gate2.sha256_bytes(quarantine.read_bytes()) != campaign_gate.EXPECTED_V3["v2_attempt_1_quarantine_sha256"]:
        raise Gate5PaidPilotRetryCampaignV3Stop("v2_terminal_evidence_invalid")
    for field, path in BUILD_PATHS.items():
        if canonical_hash(path) != campaign_gate.EXPECTED_V3[field]:
            raise Gate5PaidPilotRetryCampaignV3Stop("output_collision_build_invalid")
    components = initial_components()
    return {**pilot.validate_historical_components(components), **campaign_gate.EXPECTED_V3}


def verify_only() -> dict[str, Any]:
    evidence = verify_initial_evidence()
    if canonical_hash(PROPOSAL_PATH) != campaign_gate.EXPECTED_V3["v3_campaign_proposal_sha256"]:
        raise Gate5PaidPilotRetryCampaignV3Stop("campaign_proposal_mismatch")
    ...
    if worst_attempt != 204_000 or pilot.aggregate_pilot_cost(MAX_ATTEMPTS * worst_attempt, evidence["historical_pilot_actual_usd_millionths"]) != 1_481_400:
        raise Gate5PaidPilotRetryCampaignV3Stop("campaign_cost_math_invalid")
    return {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v3_verify_only", "all_frozen_hashes_match": True, "initial_component_count": 6, "initial_historical_cost_usd_millionths": 53_400, "attested_baseline_usd_millionths": 53_400, "maximum_campaign_attempts": MAX_ATTEMPTS, "worst_case_aggregate_usd_millionths": 1_481_400, **{field: campaign_gate.EXPECTED_V3[field] for field in BUILD_PATHS}, "network_used": False, "credential_read": False, "file_output_created": False}
...
def _state_row(event: str, sequence: int, state: str, attempts: int, context: dict[str, Any], lock_hash: str | None, completion_hash: str | None, output_name: str | None, prior: str | None, attestation_hash: str) -> dict[str, Any]:
    row = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v3_state", "event": event, "sequence": sequence, "campaign_state": state, "attempts_reserved": attempts, **context, "attempt_lock_file_sha256": lock_hash, "completion_file_sha256": completion_hash, "output_directory_name": output_name, "prior_row_hash": prior, "campaign_proposal_sha256": campaign_gate.EXPECTED_V3["v3_campaign_proposal_sha256"], "attestation_sha256": attestation_hash}
...
def _validate_state_row(row: Any, prior: str | None, attestation_hash: str) -> None:
    if not isinstance(row, dict) or set(row) != STATE_FIELDS or row["prior_row_hash"] != prior or row["attestation_sha256"] != attestation_hash or row["campaign_proposal_sha256"] != campaign_gate.EXPECTED_V3["v3_campaign_proposal_sha256"]:
        raise Gate5PaidPilotRetryCampaignV3Stop("campaign_state_invalid")
...
def load_and_verify_campaign(root: Path, attestation_hash: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, row in enumerate(rows):
        ...
        if index == 0:
            ...
            if not isinstance(lock, dict) or set(lock) != lock_fields or lock.get("row_hash") != gate2.sha256_bytes(gate2.canonical_json_bytes(lock_payload)) or canonical_hash(_lock_path(root, row["sequence"])) != row["attempt_lock_file_sha256"] or lock.get("artifact") != "gemini_generator_gate5_paid_pilot_retry_campaign_v3_attempt_lock" or lock.get("sequence") != row["sequence"] or lock.get("prior_state_row_hash") != row["prior_row_hash"] or lock.get("historical_pilot_components_sha256") != row["historical_pilot_components_sha256"] or lock.get("historical_pilot_component_count") != row["historical_pilot_component_count"] or lock.get("historical_pilot_actual_usd_millionths") != row["historical_pilot_actual_usd_millionths"] or lock.get("attestation_sha256") != attestation_hash or lock.get("output_directory_name") != row["output_directory_name"] or lock.get("state") != "reserved_before_credential_read" or gate2.contains_secret(lock):
                raise Gate5PaidPilotRetryCampaignV3Stop("campaign_state_invalid")
...
def reserve_attempt(root: Path, attestation_hash: str) -> tuple[int, list[dict[str, Any]]]:
    ...
    if last["campaign_state"] in TERMINAL_STATES:
        raise Gate5PaidPilotRetryCampaignV3Stop("campaign_already_terminal")
    if last["event"] == "attempt_reserved":
        raise Gate5PaidPilotRetryCampaignV3Stop("campaign_incomplete_attempt")
    ...
    if sequence > MAX_ATTEMPTS:
        raise Gate5PaidPilotRetryCampaignV3Stop("campaign_attempt_cap_reached")
    context = pilot.validate_historical_components(components)
    output_name = _output_path(root, sequence).name
    lock = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v3_attempt_lock", "sequence": sequence, "created_utc": utc_now(), "prior_state_row_hash": last["row_hash"], **context, "attestation_sha256": attestation_hash, "output_directory_name": output_name, "state": "reserved_before_credential_read"}
...
def _validate_attempt_output(output: Path, components: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any], str]:
    ...
    diagnostics = _load_rows(output / "output_collision_diagnostics.jsonl")
    ...
    if summary.get("summary_sha256") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)) or any(summary.get(k) != v for k, v in context.items()) or summary.get("aggregate_pilot_actual_usd_millionths") != context["historical_pilot_actual_usd_millionths"] + summary.get("cumulative_actual_usd_millionths", -1):
        raise Gate5PaidPilotRetryCampaignV3Stop("attempt_output_invalid")
    _manifest, protected_references = gate2.build_quarantine()
    labels = [label for label, _text in protected_references]
    if summary.get("global_stop") == "output_collision_diagnostic_persistence_failed":
        if diagnostics or summary.get("output_collision_diagnostic_count") != 0 or summary.get("output_collision_diagnostic_chain_head") is not None or len(rejections) != 1 or not isinstance(rejections[0].get("output_collision_diagnostic_row_hash"), str) or not rejections[0].get("reason_code", "").endswith(collision_evidence.REASON_SUFFIX):
            raise Gate5PaidPilotRetryCampaignV3Stop("attempt_output_invalid")
    else:
        collision_evidence.validate_rejection_links(rejections, diagnostics, summary, labels, collision_evidence.EXPECTED_CANDIDATE_FIELD_PATHS)
    status = receipts[-1].get("http_status") if receipts else None
    clean_503 = summary.get("candidate_quarantine_count") == 0 and summary.get("output_collision_diagnostic_count") == 0 and not diagnostics and summary.get("global_stop") == "unexpected_http_status" and status == 503 and len(receipts) == len(costs) == len(rejections) == 1 and rejections[0].get("output_collision_diagnostic_row_hash") is None
    campaign_sequence = len(components) - 5
    if clean_503:
        state = "attempt_cap_reached" if campaign_sequence == MAX_ATTEMPTS else "active_after_clean_503"
    ...
    evidence = {name: _file_hash_or_none(output / name) for name in ("pilot_reservation.json", "run_summary.json", "request_receipts.jsonl", "cost_ledger.jsonl", "rejection_ledger.jsonl", "output_collision_diagnostics.jsonl", "candidate_quarantine.jsonl")}
    evidence_sha = gate2.sha256_bytes(gate2.canonical_json_bytes(evidence))
    component = pilot._historical_component(len(components) + 1, f"campaign_v3_attempt_{campaign_sequence:03d}", summary["cumulative_actual_usd_millionths"], evidence_sha, disposition, components[-1]["row_hash"])
    completion = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v3_completion", "completion_kind": "pilot_output", "sequence": campaign_sequence, "output_directory_name": output.name, "output_evidence": evidence, "candidate_quarantine_count": summary["candidate_quarantine_count"], "completed_slots": summary["completed_slots"], "global_stop": summary["global_stop"], "terminal_http_status": status, "booked_cost_usd_millionths": summary["cumulative_actual_usd_millionths"], "campaign_state_after": state, "component": component, "component_row_hash": component["row_hash"]}
...
def _zero_request_failure_completion(output: Path, components: list[dict[str, Any]], sequence: int, stop_code: str) -> dict[str, Any]:
    ...
    evidence = {name: _file_hash_or_none(output / name) for name in ("pilot_reservation.json", "run_summary.json", "request_receipts.jsonl", "cost_ledger.jsonl", "rejection_ledger.jsonl", "output_collision_diagnostics.jsonl", "candidate_quarantine.jsonl")}
    evidence_sha = gate2.sha256_bytes(gate2.canonical_json_bytes(evidence))
    component = pilot._historical_component(len(components) + 1, f"campaign_v3_attempt_{sequence:03d}", 0, evidence_sha, "zero_request_local_failure", components[-1]["row_hash"])
    completion = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v3_completion", "completion_kind": "zero_request_local_failure", "sequence": sequence, "output_directory_name": output.name, "output_evidence": evidence, "candidate_quarantine_count": 0, "completed_slots": 0, "global_stop": stop_code, "terminal_http_status": None, "booked_cost_usd_millionths": 0, "campaign_state_after": "stopped_nonretryable_outcome", "component": component, "component_row_hash": component["row_hash"]}
    completion["row_hash"] = gate2.sha256_bytes(gate2.canonical_json_bytes(completion))
    return completion
...
def recover_incomplete_attempt(attestation_path: Path, campaign_root: Path) -> dict[str, Any]:
    ...
    except (campaign_gate.Gate5PaidPilotRetryCampaignV3AttestationError, gate2.Gate2Error) as exc:
        raise Gate5PaidPilotRetryCampaignV3Stop("recovery_precondition_failed") from exc
    if not rows or rows[-1].get("event") != "attempt_reserved" or rows[-1].get("campaign_state") != "attempt_reserved":
        raise Gate5PaidPilotRetryCampaignV3Stop("no_incomplete_attempt_to_recover")
    sequence = rows[-1]["sequence"]
    if _completion_path(campaign_root, sequence).exists():
        raise Gate5PaidPilotRetryCampaignV3Stop("recovery_output_conflict")
    output = _output_path(campaign_root, sequence)
    try:
        completion, _, _ = _validate_attempt_output(output, components)
    except Exception as exc:
        raise Gate5PaidPilotRetryCampaignV3Stop("recovery_evidence_invalid") from exc
    if completion.get("sequence") != sequence or completion.get("output_directory_name") != rows[-1].get("output_directory_name"):
        raise Gate5PaidPilotRetryCampaignV3Stop("recovery_evidence_invalid")
...
def execute_once(credential_loader: Callable[[str], str], credential_target: str, transport: Any, attestation_path: Path, rate_path: Path, campaign_root: Path) -> dict[str, Any]:
    try:
        ...
        verify_only()
        pilot.load_execution_day_rates(rate_path)
        if canonical_hash(rate_path) != attestation["execution_day_rate_snapshot_sha256"]:
            raise Gate5PaidPilotRetryCampaignV3Stop("execution_day_rate_snapshot_mismatch")
        attestation_hash = canonical_hash(attestation_path)
    ...
    except (campaign_gate.Gate5PaidPilotRetryCampaignV3AttestationError, gate2.Gate2Error, pilot.Gate5PilotStop) as exc:
        raise Gate5PaidPilotRetryCampaignV3Stop("pre_execution_validation_failed") from exc
    output = _output_path(campaign_root, sequence)
    try:
        pilot.execute_pilot(credential_loader, credential_target, transport, attestation_path, rate_path, output, historical_components=components, attestation_validator=campaign_gate.validate_attestation, attested_prior_pilot_booked_cost_usd_millionths=campaign_gate.ATTESTED_BASELINE_COST)
        completion, _, _ = _validate_attempt_output(output, components)
    except Exception as exc:
        if output.exists() and (output / "run_summary.json").exists():
            try:
                ...
                raise Gate5PaidPilotRetryCampaignV3Stop("campaign_incomplete_attempt") from exc
        else:
...
def main() -> int:
    ...
    if args.recover_incomplete:
        ...
        except Gate5PaidPilotRetryCampaignV3Stop as exc:
