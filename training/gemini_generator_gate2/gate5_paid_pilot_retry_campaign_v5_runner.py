"""Local-first v5 campaign with content-free schema evidence and reviewed resume."""
...
import gate5_paid_pilot_retry_campaign_v4_runner as v4
import gate5_paid_pilot_retry_campaign_v5_gate as campaign_gate
import gate5_paid_pilot_runner as pilot
import gate5_schema_conformance_evidence as schema_evidence


PACKAGE = Path(__file__).resolve().parent
SCOPE_NOTE_PATH = PACKAGE / "gate5_paid_pilot_retry_campaign_v5_scope_note.md"
V4_DIRECTORY = PACKAGE / "gate5_paid_pilot_retry_campaign_v4_2026-08-17"
V4_ATTESTATION = PACKAGE / "gate5_paid_pilot_retry_campaign_v4_attestation_2026-08-17.json"
...
BUILD_PATHS = {
    "v5_scope_note_sha256": SCOPE_NOTE_PATH,
    ...
    "v5_campaign_runner_sha256": PACKAGE / "gate5_paid_pilot_retry_campaign_v5_runner.py",
    "v5_focused_tests_sha256": PACKAGE / "test_gate5_paid_pilot_retry_campaign_v5.py",
}
V4_TOP_HASHES = {
    "gate5_paid_pilot_retry_campaign_v4_attestation_2026-08-17.json": campaign_gate.EXPECTED_V4["v4_final_attestation_sha256"],
    "gate5_paid_pilot_retry_campaign_v4_2026-08-17/campaign_state.jsonl": campaign_gate.EXPECTED_V4["v4_terminal_campaign_state_sha256"],
    "gate5_paid_pilot_retry_campaign_v4_2026-08-17/attempt_001_lock.json": "b59912c794226931c322551e0b1ae69a2dd50d4d70b2ae63d2b701d90cc86500",
    "gate5_paid_pilot_retry_campaign_v4_2026-08-17/attempt_001_completion.json": "477ce689150d51e046f5adbf828854dda0d36ee660dbeffa5c74ae63c2456750",
}
V4_OUTPUT_HASHES = {
    1: ["e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", "8b2ff3ecf948c54dad9b69c4e7f6343845becf2a17b9ddc2e5e9f56b6eaadb53", "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"],
}
V4_OUTPUT_NAMES = ("candidate_quarantine.jsonl", "cost_ledger.jsonl", "output_collision_diagnostics.jsonl", "pilot_reservation.json", "rejection_ledger.jsonl", "request_receipts.jsonl", "schema_conformance_diagnostics.jsonl")


class Gate5PaidPilotRetryCampaignV5Stop(RuntimeError):
...
def canonical_hash(path: Path) -> str:
    ...
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV5Stop("canonical_input_invalid") from exc


def _new_file(path: Path, data: bytes) -> None:
    try:
        with path.open("xb") as handle: handle.write(data); handle.flush(); os.fsync(handle.fileno())
    except (FileExistsError, OSError) as exc: raise Gate5PaidPilotRetryCampaignV5Stop("exclusive_output_unavailable") from exc


def _append(path: Path, row: dict[str, Any]) -> None:
    if gate2.contains_secret(row): raise Gate5PaidPilotRetryCampaignV5Stop("secret_exposure")
    try:
        with path.open("ab") as handle: handle.write(gate2.canonical_json_bytes(row)); handle.flush(); os.fsync(handle.fileno())
    except OSError as exc: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_append_failed") from exc
...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid") from exc


def initial_components() -> list[dict[str, Any]]:
    digest = canonical_hash(V4_ATTESTATION)
    rows, components = v4.load_and_verify_campaign(V4_DIRECTORY, digest)
    context = pilot.validate_historical_components(components)
    if rows[-1].get("campaign_state") != "stopped_nonretryable_outcome" or rows[-1].get("attempts_reserved") != 1 or rows[-1].get("row_hash") != campaign_gate.EXPECTED_V4["v4_terminal_state_row_sha256"]:
        raise Gate5PaidPilotRetryCampaignV5Stop("v4_terminal_state_invalid")
    if context != {"historical_pilot_component_count": 10, "historical_pilot_components_sha256": campaign_gate.INITIAL_COMPONENT_MANIFEST_SHA256, "historical_pilot_actual_usd_millionths": 85_440}:
        raise Gate5PaidPilotRetryCampaignV5Stop("initial_component_manifest_invalid")
    if components[-1].get("row_hash") != campaign_gate.EXPECTED_V4["v4_terminal_component_row_sha256"] or components[-1].get("terminal_disposition") != "zero_request_local_failure" or components[-1].get("booked_cost_usd_millionths") != 0:
        raise Gate5PaidPilotRetryCampaignV5Stop("v4_terminal_component_invalid")
    return components


def verify_initial_evidence() -> dict[str, Any]:
    for relative, expected in V4_TOP_HASHES.items():
        if canonical_hash(PACKAGE / relative) != expected: raise Gate5PaidPilotRetryCampaignV5Stop("v4_terminal_evidence_invalid")
    for sequence, expected_hashes in V4_OUTPUT_HASHES.items():
        directory = V4_DIRECTORY / f"attempt_{sequence:03d}_pilot_output"
        for name, expected in zip(V4_OUTPUT_NAMES, expected_hashes):
            path = directory / name
            actual = gate2.sha256_bytes(path.read_bytes()) if path.stat().st_size == 0 else canonical_hash(path)
            if actual != expected: raise Gate5PaidPilotRetryCampaignV5Stop("v4_terminal_evidence_invalid")
...
def verify_only() -> dict[str, Any]:
    ...
    for field, path in BUILD_PATHS.items():
        if campaign_gate.EXPECTED_BUILD[field] == "TO_BE_FILLED" or canonical_hash(path) != campaign_gate.EXPECTED_BUILD[field]:
            raise Gate5PaidPilotRetryCampaignV5Stop("v5_build_mismatch")
    ...
    if worst != 204_000 or context["historical_pilot_actual_usd_millionths"] + MAX_ATTEMPTS * worst != 697_440:
        raise Gate5PaidPilotRetryCampaignV5Stop("campaign_cost_math_invalid")
    return {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v5_verify_only", **context, "maximum_campaign_attempts": MAX_ATTEMPTS, "worst_case_aggregate_usd_millionths": 697_440, **campaign_gate.EXPECTED_BUILD, "network_used": False, "credential_read": False, "file_output_created": False}
...
def _state_row(event: str, sequence: int, state: str, attempts: int, context: dict[str, Any], lock_hash: str | None, completion_hash: str | None, output_name: str | None, pause_code: str | None, schema_hash: str | None, review_hash: str | None, rate_hash: str, rate_date: str, prior: str | None, attestation_hash: str) -> dict[str, Any]:
    row = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v5_state", "event": event, "sequence": sequence, "campaign_state": state, "attempts_reserved": attempts, **context, "attempt_lock_file_sha256": lock_hash, "completion_file_sha256": completion_hash, "output_directory_name": output_name, "pause_stop_code": pause_code, "schema_conformance_diagnostic_row_sha256": schema_hash, "review_artifact_sha256": review_hash, "effective_rate_snapshot_sha256": rate_hash, "effective_rate_date": rate_date, "prior_row_hash": prior, "campaign_proposal_sha256": campaign_gate.EXPECTED_BUILD["v5_scope_note_sha256"], "attestation_sha256": attestation_hash}
...
def _load_output_rows(path: Path) -> list[dict[str, Any]]:
    try: return [json.loads(line, object_pairs_hook=campaign_gate.reject_duplicate_keys) for line in path.read_text(encoding="utf-8").splitlines() if line]
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV5Stop("attempt_output_invalid") from exc
...
def _validate_attempt_output(output: Path, components: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any], str]:
    ...
    sequence = len(components) - campaign_gate.INITIAL_COMPONENT_COUNT + 1
...
def _zero_request_completion(output: Path, components: list[dict[str, Any]], sequence: int, stop: str) -> dict[str, Any]:
    if (output / "request_receipts.jsonl").exists() and (output / "request_receipts.jsonl").read_bytes(): raise Gate5PaidPilotRetryCampaignV5Stop("campaign_incomplete_attempt")
    names = ("pilot_reservation.json", "run_summary.json", "request_receipts.jsonl", "cost_ledger.jsonl", "rejection_ledger.jsonl", "output_collision_diagnostics.jsonl", "schema_conformance_diagnostics.jsonl", "candidate_quarantine.jsonl")
    ...
    component = pilot._historical_component(len(components) + 1, f"campaign_v5_attempt_{sequence:03d}", 0, evidence_hash, "zero_request_local_failure", components[-1]["row_hash"])
    value = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v5_completion", "completion_kind": "zero_request_local_failure", "sequence": sequence, "output_directory_name": output.name, "output_evidence": evidence, "output_evidence_sha256": evidence_hash, "candidate_quarantine_count": 0, "completed_slots": 0, "global_stop": stop, "pause_stop_code": None, "schema_conformance_diagnostic_row_sha256": None, "terminal_http_status": None, "booked_cost_usd_millionths": 0, "campaign_state_after": "stopped_nonretryable_outcome", "component": component, "component_row_hash": component["row_hash"]}
...
def _validate_state_row(row: Any, prior: str | None, attestation_hash: str) -> None:
    if not isinstance(row, dict) or set(row) != STATE_FIELDS or row.get("prior_row_hash") != prior or row.get("attestation_sha256") != attestation_hash or row.get("campaign_proposal_sha256") != campaign_gate.EXPECTED_BUILD["v5_scope_note_sha256"]: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
    payload = {key: value for key, value in row.items() if key != "row_hash"}
    if row.get("row_hash") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)) or gate2.contains_secret(row): raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
...
def load_and_verify_campaign(root: Path, attestation_hash: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, row in enumerate(rows):
        ...
        if index == 0:
            ...
            if canonical_hash(_completion_path(root, row["sequence"])) != row["completion_file_sha256"] or completion.get("row_hash") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)): raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
            ...
            if completion != derived or row["campaign_state"] != completion["campaign_state_after"] or row["pause_stop_code"] != completion["pause_stop_code"] or row["schema_conformance_diagnostic_row_sha256"] != completion["schema_conformance_diagnostic_row_sha256"]: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
            components.append(completion["component"])
        elif row["event"] == "pause_review_confirmed":
            if previous["campaign_state"] != "paused_pending_review" or row["campaign_state"] != "active_after_review" or row["sequence"] != previous["sequence"] or row["attempts_reserved"] != previous["attempts_reserved"] or not isinstance(row["review_artifact_sha256"], str): raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
            ...
            if review_lock.get("row_hash") != gate2.sha256_bytes(gate2.canonical_json_bytes(lock_payload)) or review_lock.get("paused_state_row_sha256") != previous["row_hash"] or review_lock.get("review_artifact_sha256") != row["review_artifact_sha256"]: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
            if canonical_hash(_review_path(root, row["sequence"])) != row["review_artifact_sha256"]: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
            review = gate2.load_json(_review_path(root, row["sequence"])); review_payload = {key: value for key, value in review.items() if key != "record_sha256"}; template = gate2.load_json(PACKAGE / "gate5_paid_pilot_retry_campaign_v5_review_template.json")
            if set(review) != set(template) or review.get("record_sha256") != gate2.sha256_bytes(gate2.canonical_json_bytes(review_payload)) or review.get("paused_campaign_state_row_sha256") != previous["row_hash"] or review.get("paused_attempt_sequence") != row["sequence"] or review.get("reviewed_by") != "Johnny" or review.get("pause_evidence_reviewed") is not True or review.get("resume_one_next_manual_attempt_authorized_by_johnny") is not True or review.get("effective_rate_snapshot_sha256") != row["effective_rate_snapshot_sha256"] or review.get("review_local_date") != row["effective_rate_date"] or review.get("effective_rate_tuple") != _rate_tuple(REFERENCE_RATE_PATH): raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
        else: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
        context = pilot.validate_historical_components(components)
        if any(row.get(key) != value for key, value in context.items()): raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
        prior = row["row_hash"]
...
def reserve_attempt(root: Path, attestation_hash: str, rate_path: Path) -> tuple[int, list[dict[str, Any]]]:
    ...
    if last["campaign_state"] in TERMINAL_STATES: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_already_terminal")
    if last["campaign_state"] == "paused_pending_review": raise Gate5PaidPilotRetryCampaignV5Stop("pause_review_required")
    if last["event"] == "attempt_reserved": raise Gate5PaidPilotRetryCampaignV5Stop("campaign_incomplete_attempt")
    if last["campaign_state"] not in ACTIVE_STATES: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_state_invalid")
    if local_today() != last["effective_rate_date"] or canonical_hash(rate_path) != last["effective_rate_snapshot_sha256"]: raise Gate5PaidPilotRetryCampaignV5Stop("execution_day_rate_snapshot_mismatch")
    if _rate_tuple(rate_path) != _rate_tuple(REFERENCE_RATE_PATH): raise Gate5PaidPilotRetryCampaignV5Stop("execution_day_rate_snapshot_mismatch")
    sequence = last["attempts_reserved"] + 1
    if sequence > MAX_ATTEMPTS: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_attempt_cap_reached")
    context = pilot.validate_historical_components(components); output_name = _output_path(root, sequence).name
    lock = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v5_attempt_lock", "sequence": sequence, "created_utc": utc_now(), "prior_state_row_hash": last["row_hash"], **context, "attestation_sha256": attestation_hash, "output_directory_name": output_name, "effective_rate_snapshot_sha256": last["effective_rate_snapshot_sha256"], "effective_rate_date": last["effective_rate_date"], "state": "reserved_before_credential_read"}
...
def _validate_review_artifact(path: Path, root: Path, attestation_path: Path, fresh_rate_path: Path | None) -> tuple[dict[str, Any], str, str]:
    try:
        value = json.loads(path.read_text(encoding="utf-8"), object_pairs_hook=campaign_gate.reject_duplicate_keys); template = gate2.load_json(PACKAGE / "gate5_paid_pilot_retry_campaign_v5_review_template.json")
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV5Stop("pause_review_invalid") from exc
    if not isinstance(value, dict) or set(value) != set(template): raise Gate5PaidPilotRetryCampaignV5Stop("pause_review_invalid")
    supplied_hash = value.get("record_sha256"); payload = {key: item for key, item in value.items() if key != "record_sha256"}
    if supplied_hash != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)) or gate2.contains_secret(value): raise Gate5PaidPilotRetryCampaignV5Stop("pause_review_invalid")
...
def confirm_pause_review(review_path: Path, attestation_path: Path, campaign_root: Path, fresh_rate_path: Path | None = None) -> dict[str, Any]:
    ...
    review_lock = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v5_pause_review_lock", "sequence": paused["sequence"], "paused_state_row_sha256": paused["row_hash"], "review_artifact_sha256": review_hash, "created_utc": utc_now()}
...
def _pilot_attestation_view(path: Path, execution_date: str) -> dict[str, Any]:
    """Validate v5, then expose the already-reviewed legacy engine fields."""
...
def execute_once(credential_loader: Callable[[str], str], credential_target: str, transport: Any, attestation_path: Path, rate_path: Path, campaign_root: Path) -> dict[str, Any]:
    try:
        ...
        if isinstance(exc, Gate5PaidPilotRetryCampaignV5Stop): raise
        raise Gate5PaidPilotRetryCampaignV5Stop("pre_execution_validation_failed") from exc
    ...
    try:
        ...
        if output.exists() and (output / "run_summary.json").exists():
            ...
            except Exception: raise Gate5PaidPilotRetryCampaignV5Stop("campaign_incomplete_attempt") from exc
...
def recover_incomplete_attempt(attestation_path: Path, campaign_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV5Stop("recovery_precondition_failed") from exc
    last = rows[-1]
    if last.get("event") != "attempt_reserved" or last.get("campaign_state") != "attempt_reserved": raise Gate5PaidPilotRetryCampaignV5Stop("no_incomplete_attempt_to_recover")
    sequence = last["sequence"]
    if _completion_path(campaign_root, sequence).exists(): raise Gate5PaidPilotRetryCampaignV5Stop("recovery_output_conflict")
    try: completion = _validate_attempt_output(_output_path(campaign_root, sequence), components)[0]
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV5Stop("recovery_evidence_invalid") from exc
    if completion.get("sequence") != sequence or completion.get("output_directory_name") != last.get("output_directory_name"): raise Gate5PaidPilotRetryCampaignV5Stop("recovery_evidence_invalid")
    return complete_attempt(campaign_root, sequence, components, attestation_hash, completion)
...
def main() -> int:
    ...
    except Gate5PaidPilotRetryCampaignV5Stop as exc: print(json.dumps({"disposition": "stopped", "stop_reason": exc.code}, sort_keys=True)); return 2
