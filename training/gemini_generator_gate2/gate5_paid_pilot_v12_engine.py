"""Disabled-by-default Gate 5 V12 stopword-filtered-char5gram-collision engine.

This module exposes only local verification at its command line.  Any future
real use must enter through the dedicated V12 campaign runner, which supplies
...
V12 reuses V11's (and, through it, V10's, V9's, and V8's) schedule, prompt
builder, historical legacy-tower verification, continue-past ruleset, and per-
row builders unchanged (imported directly, not duplicated).  The only
behavioral change from V11: candidate screening now calls
gate2.screen_candidate_fully_stopword_filtered() instead of
gate2.screen_candidate_stopword_filtered() -- character-5-gram Jaccard
collision scoring now also strips the same fixed, closed-set English
stopword list (preserving word order) before scoring, isolating exactly the
one additional variable Johnny asked to test next after V11's real 22/22 run
showed the identical fan-out signature on the character side that justified
fixing token-Jaccard in the first place: one real candidate matched 28
different, unrelated reference records at once, all clustered at
0.102-0.109, just over the unchanged 0.10 threshold.
Nothing else changes: TOKEN_JACCARD_THRESHOLD, CHAR_5GRAM_JACCARD_THRESHOLD,
V11's already-fixed token-Jaccard scoring, exact-match detection,
normalized-containment detection, schema validation, and the full
continue-past/hard-terminal ruleset are all byte-for-byte identical to V11.
...
import gate5_paid_pilot_v11_engine as v11
...
v9 = v11.v9
v8 = v11.v8
# V12 reuses V11's (and, through it, V10's, V9's, and V8's) own schedule/
# prompt/legacy-tower artifacts unchanged; it does not get its own schedule
# file. All of these are re-derived and pinned at load time, not trusted
# from this constant alone.
EXPECTED_V8_SCHEDULE_SHA256 = v11.EXPECTED_V8_SCHEDULE_SHA256
EXPECTED_V12_FIRST_REQUEST_SHA256 = v11.EXPECTED_V11_FIRST_REQUEST_SHA256
SCHEMA_CONFORMANCE_EVIDENCE_PATH = v11.SCHEMA_CONFORMANCE_EVIDENCE_PATH
EXPECTED_SCHEMA_CONFORMANCE_EVIDENCE_SHA256 = v11.EXPECTED_SCHEMA_CONFORMANCE_EVIDENCE_SHA256
# gate2.py is deliberately NOT hash-pinned -- an earlier revision of V11
# added a whole-file pin on it (reasoning by flawed analogy to how V10
# pinned gate5_schema_conformance_evidence.py, a small complete module that
# was never expected to change again). gate2.py is the opposite: the shared
# foundation every new version adds its own additive block to, by design.
# Pinning its whole-file hash broke V11's own re-verification for real the
# moment V12 correctly extended gate2.py further -- confirmed directly
# during this build's self-review. Removed from V11 and never added here,
# matching V6-V10's original (and correct) design. Real functional coverage
# of the specific functions V12 calls comes from V12's own test suite.
# Cost ceilings are unchanged from V11 -- this run changes collision scoring,
# not spend limits. Real per-request cost is identical to V11 (same
# schedule, same rates), so the same headroom applies.
V12_RECONCILIATION_STOP = v11.V11_RECONCILIATION_STOP
V12_PILOT_CEILING = v11.V11_PILOT_CEILING
MAX_CARD_ATTEMPTS = v11.MAX_CARD_ATTEMPTS
# Continue-past ruleset is unchanged from V11 -- confirmed explicitly with
# Johnny as out of scope for this build (char-5-gram stopword filtering
# only, schema floor deferred, everything else stays exactly as it is).
CONTINUE_PAST_REASON_SUFFIXES = v11.CONTINUE_PAST_REASON_SUFFIXES

Gate5PilotStop = v11.Gate5PilotStop
ProviderResponse = v11.ProviderResponse
HTTPSPilotTransport = v11.HTTPSPilotTransport
canonical_hash = v11.canonical_hash
utc_now = v11.utc_now
validate_provider_response = v11.validate_provider_response
load_execution_day_rates = v11.load_execution_day_rates
legacy_historical_components = v11.legacy_historical_components
validate_historical_components = v11.validate_historical_components
_historical_m01_slots = v11._historical_m01_slots
_prompt_references = v11._prompt_references
_legacy_slot_view = v11._legacy_slot_view
_new_file = v11._new_file
_append = v11._append
verify_successful_diagnostic_evidence = v11.verify_successful_diagnostic_evidence
verify_successful_flash_lite_evidence = v11.verify_successful_flash_lite_evidence
verify_fresh_attempt_evidence = v11.verify_fresh_attempt_evidence
verify_third_attempt_evidence = v11.verify_third_attempt_evidence
# Pure row builders: unchanged from V11, reused directly rather than
...
# gate2.screen_candidate_fully_stopword_filtered()'s return shape is
# identical to gate2.screen_candidate_stopword_filtered()'s).
_receipt = v11._receipt
_cost_row = v11._cost_row
_rejection = v11._rejection
_candidate_row = v11._candidate_row
_outcome_continues_past = v11._outcome_continues_past


def load_schedule() -> list[dict[str, Any]]:
    """Re-derive and pin the exact same 22-slot corrected-V8 schedule, unchanged."""
    slots = v11.load_schedule()
    if canonical_hash(v8.SCHEDULE_PATH) != EXPECTED_V8_SCHEDULE_SHA256:
        raise Gate5PilotStop("v12_schedule_source_mismatch")
    first_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(v8.v7_prompt.build_request(slots[0])))
    if first_hash != EXPECTED_V12_FIRST_REQUEST_SHA256:
        raise Gate5PilotStop("v12_request_pin_mismatch")
    return slots
...
def reservation_fits_reconciliation_stop(fresh_run_cumulative: int, next_reservation: int, historical_total: int) -> bool:
    if type(next_reservation) is not int or next_reservation < 0:
        raise Gate5PilotStop("pilot_cost_invalid")
    return aggregate_pilot_cost(fresh_run_cumulative + next_reservation, historical_total) <= V12_RECONCILIATION_STOP
...
def prepare_output_directory(root: Path, contract_hash: str, provider_schema_hash: str, rate_hash: str, attestation_hash: str, success_receipt_row_hash: str, flash_lite_success_receipt_row_hash: str, fresh_evidence: dict[str, Any], third_evidence: dict[str, Any], historical_total: int) -> dict[str, Path]:
    ...
    reservation = {
        "artifact": "gemini_generator_gate5_paid_pilot_v12_reservation",
        "created_utc": utc_now(),
        "contract_sha256": contract_hash,
        "provider_schema_sha256": provider_schema_hash,
        "v12_first_request_sha256": EXPECTED_V12_FIRST_REQUEST_SHA256,
        "v12_reused_v8_schedule_sha256": EXPECTED_V8_SCHEDULE_SHA256,
        ...
        **third_evidence,
        ...
        "pilot_ceiling_usd_millionths": V12_PILOT_CEILING,
        "reconciliation_stop_usd_millionths": V12_RECONCILIATION_STOP,
...
def execute_pilot(
    ...
    protected-collision-family outcome (now stopword-filtered on both the
    token-Jaccard and character-5-gram sides) or a genuine schema_invalid,
    retry a genuine 503 up to MAX_CARD_ATTEMPTS times on the same card, and
    stop the whole run immediately on any other exceptional outcome."""
    try:
        ...
        # A real list, not a generator -- see V9/V10/V11's identical comment;
        ...
        for card_index, slot in enumerate(slots, 1):
            ...
            for card_attempt_number in range(1, MAX_CARD_ATTEMPTS + 1):
                ...
                try:
                    ...
                    # The one behavioral change from V11: stopword-filtered
                    # character-5-gram scoring added alongside V11's already-
                    # fixed token-Jaccard, on the collision side.
                ...
                cumulative += actual
                if aggregate_pilot_cost(cumulative, historical_total) > V12_PILOT_CEILING:
                    disposition = "rejected"
    ...
    summary = {
        "artifact": "gemini_generator_gate5_paid_pilot_v12_summary",
...
def verify_local_build() -> dict[str, Any]:
    ...
    return {
        "artifact": "gemini_generator_gate5_paid_pilot_v12_engine_verify_only",
        ...
        **third_evidence,
        ...
        "v12_reused_v8_schedule_sha256": canonical_hash(v8.SCHEDULE_PATH),
        "v12_first_request_sha256": hashes[0],
        "v12_request_count": len(hashes),
        "v12_unique_request_count": len(set(hashes)),
        "max_card_attempts": MAX_CARD_ATTEMPTS,
        "network_used": False,
...
def main() -> int:
    ...
    if not args.verify_only:
        parser.error("--verify-only is required; V12 execution is available only through the campaign runner")
