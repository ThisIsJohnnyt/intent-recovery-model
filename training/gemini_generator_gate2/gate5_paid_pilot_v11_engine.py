"""Disabled-by-default Gate 5 V11 stopword-filtered-collision engine.

This module exposes only local verification at its command line.  Any future
real use must enter through the dedicated V11 campaign runner, which supplies
...
V11 reuses V10's (and, through it, V9's and V8's) schedule, prompt builder,
historical legacy-tower verification, continue-past ruleset, and per-row
builders unchanged (imported directly, not duplicated).  The only behavioral
change from V10: candidate screening now calls
gate2.screen_candidate_stopword_filtered() instead of gate2.screen_candidate()
-- token-Jaccard collision scoring strips a fixed, closed-set English
stopword list from both sides before comparing, isolating exactly the one
variable Johnny asked to test first after V10's real 22/22 run showed 64% of
cards (14/22) rejected for collision, with real diagnostic evidence showing
most of those matches clustered just over the 0.15 threshold and, in two
cases, a single candidate "colliding" with 20+ unrelated reference records at
once -- the signature of common-word inflation, not genuine duplication.
Nothing else changes: TOKEN_JACCARD_THRESHOLD, CHAR_5GRAM_JACCARD_THRESHOLD,
exact-match detection, normalized-containment detection, schema validation,
and the full continue-past/hard-terminal ruleset are all byte-for-byte
identical to V10. Reassessing the threshold values themselves, if still
warranted, is explicitly deferred to a later version, after this one
variable's real effect can be measured on its own.
"""
...
import gate5_paid_pilot_v10_engine as v10
...
v9 = v10.v9
v8 = v10.v8
# V11 reuses V10's (and, through it, V9's and V8's) own schedule/prompt/
# legacy-tower artifacts unchanged; it does not get its own schedule file.
# All of these are re-derived and pinned at load time, not trusted from this
# constant alone.
EXPECTED_V8_SCHEDULE_SHA256 = v10.EXPECTED_V8_SCHEDULE_SHA256
EXPECTED_V11_FIRST_REQUEST_SHA256 = v10.EXPECTED_V10_FIRST_REQUEST_SHA256
SCHEMA_CONFORMANCE_EVIDENCE_PATH = v10.SCHEMA_CONFORMANCE_EVIDENCE_PATH
EXPECTED_SCHEMA_CONFORMANCE_EVIDENCE_SHA256 = v10.EXPECTED_SCHEMA_CONFORMANCE_EVIDENCE_SHA256
# gate2.py itself is deliberately NOT hash-pinned here. An earlier revision
# of this file added a whole-file pin on it, reasoning by analogy to how V10
# pinned gate5_schema_conformance_evidence.py -- that analogy was wrong.
# schema_conformance_evidence.py is a small, complete module that was never
# expected to change again once its one real bug was fixed; gate2.py is the
# opposite -- it is the shared foundation every new version (V9, V10, V11,
# V12, ...) adds new additive functions to, by design. Pinning its whole-file
# hash is fundamentally incompatible with that pattern: the pin broke, for
# real, the moment V12 correctly extended gate2.py with its own additive
# block, even though V11's own behavior was completely unaffected by that
# extension. Removed here, restoring V6-V10's original (and, in hindsight,
# correct) choice never to pin it. Real functional coverage of the specific
# functions V11 calls comes from V11's own test suite instead.
# Cost ceilings are unchanged from V10 -- this run changes collision scoring,
# not spend limits. Real per-request cost is identical to V10 (same schedule,
# same rates), so the same headroom applies.
V11_RECONCILIATION_STOP = v10.V10_RECONCILIATION_STOP
V11_PILOT_CEILING = v10.V10_PILOT_CEILING
MAX_CARD_ATTEMPTS = v10.MAX_CARD_ATTEMPTS
# Continue-past ruleset is unchanged from V10 -- confirmed explicitly with
# Johnny as out of scope for this build (stopword removal only, reassess
# from there before touching anything else, thresholds and continue-past
# rules included).
CONTINUE_PAST_REASON_SUFFIXES = v10.CONTINUE_PAST_REASON_SUFFIXES

Gate5PilotStop = v10.Gate5PilotStop
ProviderResponse = v10.ProviderResponse
HTTPSPilotTransport = v10.HTTPSPilotTransport
canonical_hash = v10.canonical_hash
utc_now = v10.utc_now
validate_provider_response = v10.validate_provider_response
load_execution_day_rates = v10.load_execution_day_rates
legacy_historical_components = v10.legacy_historical_components
validate_historical_components = v10.validate_historical_components
_historical_m01_slots = v10._historical_m01_slots
_prompt_references = v10._prompt_references
_legacy_slot_view = v10._legacy_slot_view
_new_file = v10._new_file
_append = v10._append
verify_successful_diagnostic_evidence = v10.verify_successful_diagnostic_evidence
verify_successful_flash_lite_evidence = v10.verify_successful_flash_lite_evidence
verify_fresh_attempt_evidence = v10.verify_fresh_attempt_evidence
verify_third_attempt_evidence = v10.verify_third_attempt_evidence
# Pure row builders: unchanged from V10, reused directly rather than
# re-implemented (they carry no collision-scoring logic of their own -- they
# just serialize whatever screen dict execute_pilot() passes them, and
# gate2.screen_candidate_stopword_filtered()'s return shape is identical to
# gate2.screen_candidate()'s).
_receipt = v10._receipt
_cost_row = v10._cost_row
_rejection = v10._rejection
_candidate_row = v10._candidate_row
_outcome_continues_past = v10._outcome_continues_past


def load_schedule() -> list[dict[str, Any]]:
    """Re-derive and pin the exact same 22-slot corrected-V8 schedule, unchanged."""
    slots = v10.load_schedule()
    if canonical_hash(v8.SCHEDULE_PATH) != EXPECTED_V8_SCHEDULE_SHA256:
        raise Gate5PilotStop("v11_schedule_source_mismatch")
    first_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(v8.v7_prompt.build_request(slots[0])))
    if first_hash != EXPECTED_V11_FIRST_REQUEST_SHA256:
        raise Gate5PilotStop("v11_request_pin_mismatch")
    return slots
...
def reservation_fits_reconciliation_stop(fresh_run_cumulative: int, next_reservation: int, historical_total: int) -> bool:
    if type(next_reservation) is not int or next_reservation < 0:
        raise Gate5PilotStop("pilot_cost_invalid")
    return aggregate_pilot_cost(fresh_run_cumulative + next_reservation, historical_total) <= V11_RECONCILIATION_STOP
...
def prepare_output_directory(root: Path, contract_hash: str, provider_schema_hash: str, rate_hash: str, attestation_hash: str, success_receipt_row_hash: str, flash_lite_success_receipt_row_hash: str, fresh_evidence: dict[str, Any], third_evidence: dict[str, Any], historical_total: int) -> dict[str, Path]:
    ...
    reservation = {
        "artifact": "gemini_generator_gate5_paid_pilot_v11_reservation",
        "created_utc": utc_now(),
        "contract_sha256": contract_hash,
        "provider_schema_sha256": provider_schema_hash,
        "v11_first_request_sha256": EXPECTED_V11_FIRST_REQUEST_SHA256,
        "v11_reused_v8_schedule_sha256": EXPECTED_V8_SCHEDULE_SHA256,
        ...
        **third_evidence,
        ...
        "pilot_ceiling_usd_millionths": V11_PILOT_CEILING,
        "reconciliation_stop_usd_millionths": V11_RECONCILIATION_STOP,
...
def execute_pilot(
    ...
    protected-collision-family outcome (now stopword-filtered on the
    token-Jaccard side only) or a genuine schema_invalid, retry a genuine 503
    up to MAX_CARD_ATTEMPTS times on the same card, and stop the whole run
    ...
    try:
        ...
        # A real list, not a generator -- see V9/V10's identical comment; the
        # same multi-diagnostic-per-run generator-exhaustion class of bug
        ...
        for card_index, slot in enumerate(slots, 1):
            ...
            for card_attempt_number in range(1, MAX_CARD_ATTEMPTS + 1):
                ...
                try:
                    ...
                    # The one behavioral change from V10: stopword-filtered
                    # token-Jaccard scoring on the collision side. Everything
                    # else about screening is untouched.
                    screen = gate2.screen_candidate_stopword_filtered(candidate, references, earlier_candidates, prompt_references)
                ...
                cumulative += actual
                if aggregate_pilot_cost(cumulative, historical_total) > V11_PILOT_CEILING:
                    disposition = "rejected"
    ...
    summary = {
        "artifact": "gemini_generator_gate5_paid_pilot_v11_summary",
...
def verify_local_build() -> dict[str, Any]:
    ...
    return {
        "artifact": "gemini_generator_gate5_paid_pilot_v11_engine_verify_only",
        ...
        **third_evidence,
        ...
        "v11_reused_v8_schedule_sha256": canonical_hash(v8.SCHEDULE_PATH),
        "v11_first_request_sha256": hashes[0],
        "v11_request_count": len(hashes),
        "v11_unique_request_count": len(set(hashes)),
        "max_card_attempts": MAX_CARD_ATTEMPTS,
        "network_used": False,
...
def main() -> int:
    ...
    if not args.verify_only:
        parser.error("--verify-only is required; V11 execution is available only through the campaign runner")
