"""Disabled-by-default Gate 5 V8 M02-start paid-pilot engine.

This module exposes only local verification at its command line.  Any future
real use must enter through the dedicated V8 campaign runner, which supplies
...
SCHEDULE_PATH = PACKAGE / "schedule_v8_m02_start.json"
HISTORICAL_SCHEDULE_PATH = PACKAGE / "schedule.json"
EXPECTED_V8_FIRST_REQUEST_SHA256 = "d3ea4942f0b476fe149b0394b6c7dd702bca820a57e8cf6959167458d16a7283"
V8_RECONCILIATION_STOP = 3_750_000
V8_PILOT_CEILING = 5_000_000
...
def load_schedule() -> list[dict[str, Any]]:
    try:
        value = gate2.load_json(SCHEDULE_PATH)
        v7_schedule = gate2.load_json(PACKAGE / "schedule_v7.json")
        slots = value["slots"]
    except (gate2.Gate2Error, KeyError, TypeError) as exc:
        raise Gate5PilotStop("v8_schedule_unavailable") from exc
    payload = {key: item for key, item in value.items() if key != "manifest_sha256"}
    if (
        not isinstance(value, dict)
        or set(value) != {"artifact", "algorithm", "slot_count", "model_counts", "slots", "manifest_sha256"}
        or value.get("artifact") != "gemini_generator_schedule_v8_m02_start"
        or value.get("algorithm") != v7_schedule.get("algorithm")
        or value.get("slot_count") != 22
        or value.get("model_counts") != {"gemini-3.7-flash": 11, "gemini-3.5-flash-lite": 11}
        or value.get("manifest_sha256") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload))
        or not isinstance(slots, list)
        or len(slots) != 22
        or [item.get("campaign_schedule_sequence") for item in slots] != list(range(1, 23))
        or [item.get("source_schedule_slot") for item in slots] != list(range(3, 25))
        or any(item.get("prompt_collision_preflight", {}).get("fatal") is not False for item in slots)
    ):
        raise Gate5PilotStop("v8_schedule_unavailable")
    system = v7_prompt.load_v7_system_instruction()
    references = gate2.build_quarantine()[1]
    for item, old in zip(slots, v7_schedule.get("slots", [])[2:]):
        if {key: item.get(key) for key in ("source_schedule_slot", "model", "mechanism_id")} != {"source_schedule_slot": old.get("slot"), "model": old.get("model"), "mechanism_id": old.get("mechanism_id")}:
            raise Gate5PilotStop("v8_schedule_historical_fallback")
        user = gate2.render_messages(item["mechanism_id"])[1]
        expected_prompt_hash = gate2.sha256_bytes(gate2.canonical_json_bytes({"system_instruction": system, "user_message": user}))
        expected_preflight = gate2.collision_check(system + "\n" + user, references)
        if item.get("prompt_hash") != expected_prompt_hash or item.get("prompt_collision_preflight") != expected_preflight:
            raise Gate5PilotStop("v8_schedule_historical_fallback")
    first_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(v7_prompt.build_request(slots[0])))
    if first_hash != EXPECTED_V8_FIRST_REQUEST_SHA256:
        raise Gate5PilotStop("v8_request_pin_mismatch")
    return slots


def _historical_m01_slots() -> tuple[dict[str, Any], dict[str, Any]]:
    """Return the true historical M01 slot pair (schedule.json slots 1-2), used only to
    re-verify the legacy V1-era compatibility envelope pins. V8 deliberately excludes M01
    from its own load_schedule() output, so that function cannot be reused here - doing so
    was the exact defect behind the 2026-08-17 zero-cost real-attempt failure
    (attestation_artifact_hash_mismatch). This function never builds or sends a real V8
    request; the real per-attempt loop continues to use load_schedule() and
    v7_prompt.build_request() exclusively."""
    if canonical_hash(HISTORICAL_SCHEDULE_PATH) != execution_gate.EXPECTED_SCHEDULE_SHA256:
        raise Gate5PilotStop("historical_schedule_hash_mismatch")
    try:
        schedule = gate2.load_json(HISTORICAL_SCHEDULE_PATH)
        slots = schedule["slots"]
    except (gate2.Gate2Error, KeyError, TypeError) as exc:
        raise Gate5PilotStop("historical_schedule_unavailable") from exc
    if (
        not isinstance(slots, list)
        or len(slots) != 24
        or slots[0].get("mechanism_id") != "M01"
        or slots[1].get("mechanism_id") != "M01"
        or {slots[0].get("model"), slots[1].get("model")} != {"gemini-3.7-flash", "gemini-3.5-flash-lite"}
    ):
        raise Gate5PilotStop("historical_schedule_slot_mismatch")
...
def reservation_fits_reconciliation_stop(fresh_run_cumulative: int, next_reservation: int, historical_total: int = PRIOR_PILOT_BOOKED_COST) -> bool:
    if type(next_reservation) is not int or next_reservation < 0:
        raise Gate5PilotStop("pilot_cost_invalid")
    return aggregate_pilot_cost(fresh_run_cumulative + next_reservation, historical_total) <= V8_RECONCILIATION_STOP
...
def verify_local_build() -> dict[str, Any]:
    ...
    v8_slots = load_schedule()
    v8_hashes = [gate2.sha256_bytes(gate2.canonical_json_bytes(v7_prompt.build_request(slot))) for slot in v8_slots]
    ...
    # Recompute the legacy compatibility envelopes the same way execute_pilot() now does,
    # rather than echoing the expected constants unverified - this is what --verify-only
    # failed to catch before the 2026-08-17 real-attempt defect (execute_pilot() derived
    # these from V8's own M02-start load_schedule() instead of the true historical M01 pair).
    historical_slot_one, historical_slot_two = _historical_m01_slots()
    live_request_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(redesign.build_request(historical_slot_one)))
    flash_lite_request_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(redesign.build_request(historical_slot_two)))
    ...
    return {
        "artifact": "gemini_generator_gate5_paid_pilot_v8_engine_verify_only",
        ...
        **third_evidence,
        ...
        "v8_schedule_sha256": canonical_hash(SCHEDULE_PATH),
        "v8_first_request_sha256": v8_hashes[0],
        "v8_request_count": len(v8_hashes),
        "v8_unique_request_count": len(set(v8_hashes)),
        "network_used": False,
...
def verify_fresh_attempt_evidence() -> dict[str, Any]:
    evidence = {
        ...
        **verify_prior_failed_pilot_evidence(),
        "schedule_sha256": canonical_hash(PACKAGE / "schedule.json"),
    }
...
def prepare_output_directory(root: Path, contract_hash: str, provider_schema_hash: str, rate_hash: str, attestation_hash: str, success_receipt_row_hash: str, flash_lite_success_receipt_row_hash: str, fresh_evidence: dict[str, Any], third_evidence: dict[str, Any], historical_context: dict[str, Any] | None = None) -> dict[str, Path]:
    ...
    reservation = {
        ...
        "v8_first_request_sha256": EXPECTED_V8_FIRST_REQUEST_SHA256,
        "v8_schedule_sha256": canonical_hash(SCHEDULE_PATH),
        ...
        **third_evidence,
        ...
        "slot_count": 22,
        "pilot_ceiling_usd_millionths": V8_PILOT_CEILING,
        "reconciliation_stop_usd_millionths": V8_RECONCILIATION_STOP,
...
def _prompt_references(slots: list[dict[str, Any]]) -> list[tuple[str, str]]:
    ...
    system = v7_prompt.load_v7_system_instruction()
    for slot in slots:
        user = gate2.render_messages(slot["mechanism_id"])[1]
        result.append((f"prompt:v8:{slot['campaign_schedule_sequence']}", system + "\n" + user))
    return result
...
def _legacy_slot_view(slot: dict[str, Any]) -> dict[str, Any]:
    """Present a V8 schedule slot in the shape the shared V1-era evidence-builder modules
    (gate5_schema_conformance_evidence.build_row, gate5_output_collision_evidence.build_row)
    still require - keyed by "slot", not V8's campaign_schedule_sequence/source_schedule_slot
    split. Those shared modules are used by every version and must not be edited to learn V8's
    shape; this adapter is local to V8 only and never changes what the diagnostic row records
    (schedule_slot always equals the real campaign sequence, exactly like _receipt/_cost_row/
    _candidate_row already do). Discovered via V8's own added end-to-end test coverage: without
    this, any real schema_invalid or protected-collision outcome on V8 would silently downgrade
    to *_withheld and lose the very diagnostic evidence the M02-start campaign exists to produce."""
    return {"slot": slot["campaign_schedule_sequence"], "model": slot["model"], "mechanism_id": slot["mechanism_id"]}
...
def execute_pilot(
    ...
    private_diagnostic_sequence: int | None = None,
) -> dict[str, Any]:
    """Execute the frozen 22-slot M02-start plan only after every local gate has passed."""
    try:
        ...
        third_evidence = verify_third_attempt_evidence()
        historical_slot_one, historical_slot_two = _historical_m01_slots()
        first_request_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(redesign.build_request(historical_slot_one)))
        flash_lite_request_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(redesign.build_request(historical_slot_two)))
    ...
    try:
        for sequence, slot in enumerate(slots, 1):
            request = v7_prompt.build_request(slot)
            ...
            try:
                ...
                stop_reason = str(exc)
                if private_diagnostic_root is not None and private_diagnostic_sequence is not None and raw_hash is not None:
                ...
                if stop_reason == "schema_invalid":
                    ...
                    if schema_error is None or raw_hash is None:
                        ...
                        try:
                            pending_schema_diagnostic = schema_evidence.build_row(sequence, _legacy_slot_view(slot), request_hash, raw_hash, schema_error, prior_schema_diagnostic)
            ...
            cumulative += actual
            if aggregate_pilot_cost(cumulative, historical_context["historical_pilot_actual_usd_millionths"]) > V8_PILOT_CEILING:
                disposition = "rejected"
                ...
                if raw_output_diagnostic_state == "persisted":
                    try:
                        private_row = raw_diagnostic.persist(
                            ...
                            private_diagnostic_sequence,
                            stop_reason or "",
                            request_hash,
                            raw_hash or "",
                            rejection["row_hash"],
                            pending_raw_output_diagnostic["text"],
                        )
...
def main() -> int:
    ...
    if not args.verify_only:
        parser.error("--verify-only is required; V8 execution is available only through the campaign runner")
