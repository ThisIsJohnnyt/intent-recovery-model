...
import gate2
import gate5_paid_pilot_retry_campaign_v5_gate as gate
import gate5_paid_pilot_retry_campaign_v5_runner as runner
...
class Gate5PaidPilotRetryCampaignV5Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.valid = {
    ...
    def attestation_stub(self, root: Path, rate: Path, execution_date: str = "2026-08-17") -> tuple[Path, dict]:
        value = gate2.load_json(PACKAGE / "gate5_paid_pilot_retry_campaign_v5_attestation_template.json")
        value.update({**gate.EXPECTED_BUILD, **gate.EXPECTED_V4})
    ...
    def review(self, root: Path, campaign: Path, attestation: Path, mode: str, today: str, fresh_rate: Path | None = None) -> Path:
        ...
        value = gate2.load_json(PACKAGE / "gate5_paid_pilot_retry_campaign_v5_review_template.json")
        value.update({"review_mode": mode, "pause_local_date": paused["effective_rate_date"], "review_local_date": today, "v5_final_attestation_sha256": runner.canonical_hash(attestation), "v5_scope_note_sha256": gate.EXPECTED_BUILD["v5_scope_note_sha256"], "paused_campaign_state_row_sha256": paused["row_hash"], "paused_attempt_sequence": paused["sequence"], "paused_attempt_lock_sha256": paused["attempt_lock_file_sha256"], "paused_attempt_completion_sha256": paused["completion_file_sha256"], "paused_output_evidence_sha256": completion["output_evidence_sha256"], "pause_stop_code": paused["pause_stop_code"], "schema_conformance_diagnostic_row_sha256": paused["schema_conformance_diagnostic_row_sha256"], "current_attempts_reserved": paused["attempts_reserved"], "current_attempts_remaining": 3 - paused["attempts_reserved"], "current_historical_component_count": context["historical_pilot_component_count"], "current_historical_components_sha256": context["historical_pilot_components_sha256"], "current_historical_actual_usd_millionths": context["historical_pilot_actual_usd_millionths"], "prior_rate_snapshot_sha256": paused["effective_rate_snapshot_sha256"], "prior_rate_tuple": prior_tuple})
    ...
    def test_v4_is_rederived_and_immutable(self) -> None:
        before = {relative: runner.canonical_hash(PACKAGE / relative) for relative in runner.V4_TOP_HASHES}
        context = runner.verify_initial_evidence(); self.assertEqual(context["historical_pilot_actual_usd_millionths"], 85_440); self.assertEqual(context["historical_pilot_component_count"], 10); self.assertEqual(context["historical_pilot_components_sha256"], gate.INITIAL_COMPONENT_MANIFEST_SHA256)
        self.assertEqual(before, {relative: runner.canonical_hash(PACKAGE / relative) for relative in runner.V4_TOP_HASHES})

    def test_verify_only_and_attestation_remain_execution_gated(self) -> None:
        result = runner.verify_only(); self.assertFalse(result["network_used"] or result["credential_read"] or result["file_output_created"]); self.assertEqual(result["maximum_campaign_attempts"], 3); self.assertEqual(result["worst_case_aggregate_usd_millionths"], 697_440)
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            root = Path(temp); rates = self.rates(root); attestation, _ = self.attestation_stub(root, rates); value = gate2.load_json(attestation); value["v5_campaign_execution_authorized_by_johnny"] = False; attestation.write_bytes(gate2.canonical_json_bytes(value))
            with self.assertRaisesRegex(gate.Gate5PaidPilotRetryCampaignV5AttestationError, "unconfirmed"): gate.validate_attestation(attestation)
    ...
    def test_only_whitelist_pauses_and_schema_requires_diagnostic(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with self.assertRaises(runner.Gate5PaidPilotRetryCampaignV5Stop): runner._validate_attempt_output(root / "schema_invalid", components)
    ...
    def test_same_day_review_is_one_use_and_no_credential(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with patch.object(runner, "local_today", return_value="2026-08-17"):
                ...
                with self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV5Stop, "pause_review_invalid"): runner.confirm_pause_review(review, attestation, campaign)
    ...
    def test_next_day_review_accepts_equal_rates_and_rejects_drift(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with patch.object(runner, "local_today", return_value="2026-08-17"), self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV5Stop, "pause_review_invalid"): runner.confirm_pause_review(review, attestation, campaign, fresh)
    ...
    def test_next_day_review_left_until_day_n_plus_2_fails_at_reservation(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with patch.object(runner, "local_today", return_value="2026-08-18"), self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV5Stop, "execution_day_rate_snapshot_mismatch"): runner.reserve_attempt(campaign, runner.canonical_hash(attestation), fresh)
    ...
    def test_third_attempt_is_terminal_and_money_is_monotonic(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            root = Path(temp); components = runner.initial_components()
            for sequence in range(1, 3): components.append(pilot._historical_component(len(components) + 1, f"prior-v5-{sequence}", 204_000, "d" * 64, "transient_http_503", components[-1]["row_hash"]))
            output = root / "third"; self.fake_output(output, components, "503"); completion = runner._validate_attempt_output(output, components)[0]
            self.assertEqual(completion["campaign_state_after"], "attempt_cap_reached"); final = pilot.validate_historical_components([*components, completion["component"]]); self.assertEqual(final["historical_pilot_actual_usd_millionths"], 85_440 + 2 * 204_000 + 10_680); self.assertLessEqual(final["historical_pilot_actual_usd_millionths"], 697_440)

    def test_old_v4_attestation_cannot_pass_v5_gate(self) -> None:
        with self.assertRaises(gate.Gate5PaidPilotRetryCampaignV5AttestationError):
            gate.validate_attestation(runner.V4_ATTESTATION)
    ...
    def test_credential_failure_is_zero_cost_evidenced_and_terminal(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            rows, components = runner.load_and_verify_campaign(campaign, runner.canonical_hash(attestation)); self.assertEqual(rows[-1]["campaign_state"], "stopped_nonretryable_outcome"); self.assertEqual(pilot.validate_historical_components(components)["historical_pilot_actual_usd_millionths"], 85_440)
    ...
    def test_review_field_tampering_fails_without_state_mutation(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with patch.object(runner, "local_today", return_value="2026-08-17"), self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV5Stop, "pause_review_invalid"): runner.confirm_pause_review(review, attestation, campaign)
    ...
    def test_recovery_uses_existing_evidence_and_never_resends(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            with self.assertRaisesRegex(runner.Gate5PaidPilotRetryCampaignV5Stop, "no_incomplete_attempt_to_recover"): runner.recover_incomplete_attempt(attestation, campaign)
    ...
    def test_concurrent_attempt_reservation_has_one_winner(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            self.assertEqual(sum(isinstance(item, runner.Gate5PaidPilotRetryCampaignV5Stop) for item in outcomes), 2)
