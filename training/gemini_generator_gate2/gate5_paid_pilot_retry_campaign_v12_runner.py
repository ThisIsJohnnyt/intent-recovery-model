"""Local-first V12 full-schedule campaign runner. Single authorized execution
processes the entire 22-card schedule; there is no multi-attempt reservation
or pause/review cycle, same shape as V9/V10/V11. Live progress is printed
per card as the run proceeds. The one behavioral change from V11: candidate
screening now also uses stopword-filtered character-5-gram collision
scoring, alongside V11's already-fixed token-Jaccard scoring."""
...
import gate5_paid_pilot_retry_campaign_v11_runner as v11runner
import gate5_paid_pilot_retry_campaign_v12_gate as campaign_gate
import gate5_paid_pilot_v8_engine as v8_engine
import gate5_paid_pilot_v12_engine as pilot


PACKAGE = Path(__file__).resolve().parent
V11_CAMPAIGN_DIRECTORY = PACKAGE / "gate5_paid_pilot_retry_campaign_v11_2026-08-18"
V11_ATTESTATION = PACKAGE / "gate5_paid_pilot_retry_campaign_v11_attestation_2026-08-18.json"
REFERENCE_RATE_PATH = PACKAGE / "gate5_execution_day_rate_snapshot_2026-08-18.json"
LEDGER_NAME = "campaign_state.jsonl"
CAMPAIGN_DIRECTORY_RE = re.compile(r"gate5_paid_pilot_retry_campaign_v12_(\d{4}-\d{2}-\d{2})(?:_r[2-9])?")
# The exact 8 evidence files V11's real run_output directory wrote, in the
# same order used to build every prior version's historical-component
# evidence hash.
V11_EVIDENCE_FILE_NAMES = ("pilot_reservation.json", "run_summary.json", "request_receipts.jsonl", "cost_ledger.jsonl", "rejection_ledger.jsonl", "output_collision_diagnostics.jsonl", "schema_conformance_diagnostics.jsonl", "candidate_quarantine.jsonl")


class Gate5PaidPilotRetryCampaignV12Stop(RuntimeError):
...
def canonical_hash(path: Path) -> str:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV12Stop("canonical_input_invalid") from exc
...
def _rate_tuple(path: Path) -> dict[str, Any]:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV12Stop("execution_day_rate_snapshot_invalid") from exc
...
def v11_terminal_components() -> list[dict[str, Any]]:
    """Re-derive V10's real 27-component historical list (unchanged, exactly
    as V11 itself used it), independently re-verify V11's own real terminal
    run, and append V11's real spend as a new chained historical component --
    the same "each version's real terminal outcome becomes the next
    version's historical component" pattern V9, V10, and V11 all used."""
    v10_components = v11runner.v10_terminal_components()
    rows = v11runner.load_and_verify_campaign(V11_CAMPAIGN_DIRECTORY, canonical_hash(V11_ATTESTATION))
    last = rows[-1]
    if last.get("event") != "run_completed" or last.get("campaign_state") != "completed_full_schedule":
        raise Gate5PaidPilotRetryCampaignV12Stop("v11_terminal_evidence_invalid")
    if canonical_hash(V11_ATTESTATION) != campaign_gate.EXPECTED_V11["v11_final_attestation_sha256"]:
        raise Gate5PaidPilotRetryCampaignV12Stop("v11_terminal_evidence_invalid")
    if canonical_hash(V11_CAMPAIGN_DIRECTORY / LEDGER_NAME) != campaign_gate.EXPECTED_V11["v11_terminal_campaign_ledger_sha256"]:
        raise Gate5PaidPilotRetryCampaignV12Stop("v11_terminal_evidence_invalid")
    if last.get("run_summary_sha256") != campaign_gate.EXPECTED_V11["v11_terminal_run_summary_sha256"] or last.get("row_hash") != campaign_gate.EXPECTED_V11["v11_terminal_state_row_sha256"]:
        raise Gate5PaidPilotRetryCampaignV12Stop("v11_terminal_evidence_invalid")
    if last.get("historical_pilot_components_sha256") != campaign_gate.EXPECTED_V11["v11_terminal_historical_component_manifest_sha256"]:
        raise Gate5PaidPilotRetryCampaignV12Stop("v11_terminal_evidence_invalid")
    output = V11_CAMPAIGN_DIRECTORY / last["output_directory_name"]
    evidence = {name: _file_hash_or_none(output / name) for name in V11_EVIDENCE_FILE_NAMES}
    evidence_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(evidence))
    summary = gate2.load_json(output / "run_summary.json")
    if summary.get("cumulative_actual_usd_millionths") != 369_448 or summary.get("global_stop") != "completed_full_schedule":
        raise Gate5PaidPilotRetryCampaignV12Stop("v11_terminal_evidence_invalid")
    new_component = v8_engine._historical_component(
        len(v10_components) + 1,
        "campaign_v11_full_schedule_2026-08-18",
        summary["cumulative_actual_usd_millionths"],
        evidence_hash,
        "completed_full_schedule",
        v10_components[-1]["row_hash"],
    )
    components = v10_components + [new_component]
...
def verify_initial_evidence() -> dict[str, Any]:
    return pilot.validate_historical_components(v11_terminal_components())
...
def verify_only() -> dict[str, Any]:
    ...
    slots = pilot.load_schedule()  # raises if EXPECTED_V12_FIRST_REQUEST_SHA256 / EXPECTED_V8_SCHEDULE_SHA256 drift
    single_pass = sum(gate2.reservation_cost(slot["model"], rates) for slot in slots)
    worst = single_pass * pilot.MAX_CARD_ATTEMPTS
    if single_pass != 187_000 or worst != 935_000 or context["historical_pilot_actual_usd_millionths"] + worst != 1_989_949:
        raise Gate5PaidPilotRetryCampaignV12Stop("campaign_cost_math_invalid")
    return {
        "artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v12_verify_only",
        **context,
        "single_pass_reservation_usd_millionths": single_pass,
        "worst_case_aggregate_usd_millionths": 1_989_949,
        "max_card_attempts": pilot.MAX_CARD_ATTEMPTS,
        **campaign_gate.EXPECTED_V11,
        **build_hashes,
        "network_used": False,
...
def _validate_campaign_root(root: Path, execution_date: str) -> None:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV12Stop("campaign_directory_invalid") from exc
    ...
    if resolved.parent != PACKAGE.resolve() or match is None or match.group(1) != execution_date:
        raise Gate5PaidPilotRetryCampaignV12Stop("campaign_directory_invalid")
...
def _state_row(event: str, sequence: int, state: str, context: dict[str, Any], rate_hash: str, rate_date: str, run_summary_hash: str | None, output_directory_name: str | None, prior: str | None, attestation_hash: str) -> dict[str, Any]:
    row = {
        "artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v12_state",
        "event": event,
        "sequence": sequence,
...
def load_and_verify_campaign(root: Path, attestation_hash: str) -> list[dict[str, Any]]:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV12Stop("campaign_state_missing") from exc
    rows = [json.loads(line) for line in text.splitlines() if line.strip()]
    if not rows:
        raise Gate5PaidPilotRetryCampaignV12Stop("campaign_state_invalid")
    prior = None
...
def execute_full_schedule(credential_loader: Callable[[str], str], credential_target: str, transport: Any, attestation_path: Path, rate_path: Path, campaign_root: Path, print_progress: Callable[[dict[str, Any]], None] | None = None) -> dict[str, Any]:
    if campaign_root.exists():
        raise Gate5PaidPilotRetryCampaignV12Stop("campaign_directory_already_exists")
    try:
        ...
        verify_only()
        components = v11_terminal_components()
        ...
        if rate_hash != attestation["execution_day_rate_snapshot_sha256"]:
            raise Gate5PaidPilotRetryCampaignV12Stop("execution_day_rate_snapshot_mismatch")
    except Exception as exc:
        if isinstance(exc, Gate5PaidPilotRetryCampaignV12Stop):
            raise
        raise Gate5PaidPilotRetryCampaignV12Stop("pre_execution_validation_failed") from exc
    ...
    try:
        summary = pilot.execute_pilot(
            ...
            attestation_validator=lambda _p: _v11_style_legacy_attestation(components),
            effective_rate_snapshot_sha256=rate_hash,
        ...
        raise Gate5PaidPilotRetryCampaignV12Stop(code) from exc
...
def _v11_style_legacy_attestation(components: list[dict[str, Any]]) -> dict[str, Any]:
    """Same substitution pattern V8/V9/V10/V11 use: validate the real V12
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except Gate5PaidPilotRetryCampaignV12Stop as exc:
