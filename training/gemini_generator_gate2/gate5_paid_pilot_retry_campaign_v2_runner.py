"""Disabled-by-default bounded manual campaign v2 for full Gate 5 pilot attempts."""
...
import gate5_paid_pilot_retry_campaign_v2_gate as campaign_gate
import gate5_paid_pilot_retry_campaign_runner as v1
import gate5_paid_pilot_runner as pilot

PACKAGE = Path(__file__).resolve().parent
PROPOSAL_PATH = PACKAGE / "gate5_paid_pilot_retry_campaign_v2_proposal.md"
RATE_PATH = PACKAGE / "gate5_execution_day_rate_snapshot_2026-08-16.json"
V1_DIRECTORY = PACKAGE / "gate5_paid_pilot_retry_campaign_2026-08-16"
V1_ATTESTATION = PACKAGE / "gate5_paid_pilot_retry_campaign_attestation_2026-08-16.json"
...
class Gate5PaidPilotRetryCampaignV2Stop(RuntimeError):
...
def canonical_hash(path: Path) -> str:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV2Stop("canonical_input_invalid") from exc
...
def _new_file(path: Path, data: bytes) -> None:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV2Stop("exclusive_output_unavailable") from exc
...
def _append(path: Path, row: dict[str, Any]) -> None:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV2Stop("campaign_state_append_failed") from exc
...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    except (OSError, UnicodeDecodeError, json.JSONDecodeError, campaign_gate.Gate5PaidPilotRetryCampaignV2AttestationError) as exc:
        raise Gate5PaidPilotRetryCampaignV2Stop("campaign_state_invalid") from exc


def initial_components() -> list[dict[str, Any]]:
    attestation_hash = canonical_hash(V1_ATTESTATION)
    rows, components = v1.load_and_verify_campaign(V1_DIRECTORY, attestation_hash)
    context = pilot.validate_historical_components(components)
    if rows[-1].get("campaign_state") != "stopped_nonretryable_outcome" or rows[-1].get("attempts_reserved") != 2:
        raise Gate5PaidPilotRetryCampaignV2Stop("v1_terminal_state_invalid")
    if context != {"historical_pilot_component_count": 5, "historical_pilot_components_sha256": campaign_gate.INITIAL_COMPONENT_MANIFEST_SHA256, "historical_pilot_actual_usd_millionths": 42_720}:
        raise Gate5PaidPilotRetryCampaignV2Stop("initial_component_manifest_invalid")
    if components[-1].get("booked_cost_usd_millionths") != 0 or components[-1].get("terminal_disposition") != "zero_request_local_failure":
        raise Gate5PaidPilotRetryCampaignV2Stop("v1_terminal_component_invalid")
    return components
...
def _load_rows(path: Path) -> list[dict[str, Any]]:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV2Stop("historical_pilot_evidence_invalid") from exc
...
def _verify_summary(directory: Path, expected: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], bytes]:
    ...
    if summary.get("summary_sha256") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)):
        raise Gate5PaidPilotRetryCampaignV2Stop("historical_pilot_evidence_invalid")
...
def verify_initial_evidence() -> dict[str, Any]:
    paths = {
        "v1_final_attestation_sha256": V1_ATTESTATION,
        "v1_terminal_campaign_state_sha256": V1_DIRECTORY / v1.LEDGER_NAME,
        "v1_attempt_1_lock_sha256": V1_DIRECTORY / "attempt_001_lock.json",
        "v1_attempt_1_completion_sha256": V1_DIRECTORY / "attempt_001_completion.json",
        "v1_attempt_1_reservation_sha256": V1_DIRECTORY / "attempt_001_pilot_output" / "pilot_reservation.json",
        "v1_attempt_1_summary_sha256": V1_DIRECTORY / "attempt_001_pilot_output" / "run_summary.json",
        "v1_attempt_1_receipts_sha256": V1_DIRECTORY / "attempt_001_pilot_output" / "request_receipts.jsonl",
        "v1_attempt_1_cost_sha256": V1_DIRECTORY / "attempt_001_pilot_output" / "cost_ledger.jsonl",
        "v1_attempt_1_rejection_sha256": V1_DIRECTORY / "attempt_001_pilot_output" / "rejection_ledger.jsonl",
        "v1_attempt_2_lock_sha256": V1_DIRECTORY / "attempt_002_lock.json",
        "v1_attempt_2_completion_sha256": V1_DIRECTORY / "attempt_002_completion.json",
    }
    for field, path in paths.items():
        if canonical_hash(path) != campaign_gate.EXPECTED_V2[field]:
            raise Gate5PaidPilotRetryCampaignV2Stop("v1_terminal_evidence_invalid")
    quarantine = V1_DIRECTORY / "attempt_001_pilot_output" / "candidate_quarantine.jsonl"
    if gate2.sha256_bytes(quarantine.read_bytes()) != campaign_gate.EXPECTED_V2["v1_attempt_1_quarantine_sha256"]:
        raise Gate5PaidPilotRetryCampaignV2Stop("v1_terminal_evidence_invalid")
    components = initial_components()
    return {**pilot.validate_historical_components(components), **campaign_gate.EXPECTED_V2}


def verify_only() -> dict[str, Any]:
    evidence = verify_initial_evidence()
    if canonical_hash(PROPOSAL_PATH) != campaign_gate.EXPECTED_V2["v2_campaign_proposal_sha256"]:
        raise Gate5PaidPilotRetryCampaignV2Stop("campaign_proposal_mismatch")
    ...
    if worst_attempt != 204_000 or pilot.aggregate_pilot_cost(MAX_ATTEMPTS * worst_attempt, evidence["historical_pilot_actual_usd_millionths"]) != 1_674_720:
        raise Gate5PaidPilotRetryCampaignV2Stop("campaign_cost_math_invalid")
    return {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v2_verify_only", "all_frozen_hashes_match": True, "initial_component_count": 5, "initial_historical_cost_usd_millionths": 42_720, "attested_baseline_usd_millionths": 32_040, "maximum_campaign_attempts": MAX_ATTEMPTS, "worst_case_aggregate_usd_millionths": 1_674_720, "network_used": False, "credential_read": False, "file_output_created": False}
...
def _state_row(event: str, sequence: int, state: str, attempts: int, context: dict[str, Any], lock_hash: str | None, completion_hash: str | None, output_name: str | None, prior: str | None, attestation_hash: str) -> dict[str, Any]:
    row = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v2_state", "event": event, "sequence": sequence, "campaign_state": state, "attempts_reserved": attempts, **context, "attempt_lock_file_sha256": lock_hash, "completion_file_sha256": completion_hash, "output_directory_name": output_name, "prior_row_hash": prior, "campaign_proposal_sha256": campaign_gate.EXPECTED_V2["v2_campaign_proposal_sha256"], "attestation_sha256": attestation_hash}
...
def _validate_state_row(row: Any, prior: str | None, attestation_hash: str) -> None:
    if not isinstance(row, dict) or set(row) != STATE_FIELDS or row["prior_row_hash"] != prior or row["attestation_sha256"] != attestation_hash or row["campaign_proposal_sha256"] != campaign_gate.EXPECTED_V2["v2_campaign_proposal_sha256"]:
        raise Gate5PaidPilotRetryCampaignV2Stop("campaign_state_invalid")
...
def load_and_verify_campaign(root: Path, attestation_hash: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, row in enumerate(rows):
        ...
        if index == 0:
            ...
            if not isinstance(lock, dict) or set(lock) != lock_fields or lock.get("row_hash") != gate2.sha256_bytes(gate2.canonical_json_bytes(lock_payload)) or canonical_hash(_lock_path(root, row["sequence"])) != row["attempt_lock_file_sha256"] or lock.get("artifact") != "gemini_generator_gate5_paid_pilot_retry_campaign_v2_attempt_lock" or lock.get("sequence") != row["sequence"] or lock.get("prior_state_row_hash") != row["prior_row_hash"] or lock.get("historical_pilot_components_sha256") != row["historical_pilot_components_sha256"] or lock.get("historical_pilot_component_count") != row["historical_pilot_component_count"] or lock.get("historical_pilot_actual_usd_millionths") != row["historical_pilot_actual_usd_millionths"] or lock.get("attestation_sha256") != attestation_hash or lock.get("output_directory_name") != row["output_directory_name"] or lock.get("state") != "reserved_before_credential_read" or gate2.contains_secret(lock):
                raise Gate5PaidPilotRetryCampaignV2Stop("campaign_state_invalid")
...
def reserve_attempt(root: Path, attestation_hash: str) -> tuple[int, list[dict[str, Any]]]:
    ...
    if last["campaign_state"] in TERMINAL_STATES:
        raise Gate5PaidPilotRetryCampaignV2Stop("campaign_already_terminal")
    if last["event"] == "attempt_reserved":
        raise Gate5PaidPilotRetryCampaignV2Stop("campaign_incomplete_attempt")
    ...
    if sequence > MAX_ATTEMPTS:
        raise Gate5PaidPilotRetryCampaignV2Stop("campaign_attempt_cap_reached")
    context = pilot.validate_historical_components(components)
    output_name = _output_path(root, sequence).name
    lock = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v2_attempt_lock", "sequence": sequence, "created_utc": utc_now(), "prior_state_row_hash": last["row_hash"], **context, "attestation_sha256": attestation_hash, "output_directory_name": output_name, "state": "reserved_before_credential_read"}
...
def _validate_attempt_output(output: Path, components: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any], str]:
    ...
    if summary.get("summary_sha256") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)) or any(summary.get(k) != v for k, v in context.items()) or summary.get("aggregate_pilot_actual_usd_millionths") != context["historical_pilot_actual_usd_millionths"] + summary.get("cumulative_actual_usd_millionths", -1):
        raise Gate5PaidPilotRetryCampaignV2Stop("attempt_output_invalid")
    ...
    campaign_sequence = len(components) - 4
    if clean_503:
...
def _zero_request_failure_completion(output: Path, components: list[dict[str, Any]], sequence: int, stop_code: str) -> dict[str, Any]:
    ...
    evidence = {name: _file_hash_or_none(output / name) for name in ("pilot_reservation.json", "run_summary.json", "request_receipts.jsonl", "cost_ledger.jsonl", "rejection_ledger.jsonl", "candidate_quarantine.jsonl")}
    evidence_sha = gate2.sha256_bytes(gate2.canonical_json_bytes(evidence))
    component = pilot._historical_component(len(components) + 1, f"campaign_v2_attempt_{sequence:03d}", 0, evidence_sha, "zero_request_local_failure", components[-1]["row_hash"])
...
def recover_incomplete_attempt(attestation_path: Path, campaign_root: Path) -> dict[str, Any]:
    ...
    except (campaign_gate.Gate5PaidPilotRetryCampaignV2AttestationError, gate2.Gate2Error) as exc:
        raise Gate5PaidPilotRetryCampaignV2Stop("recovery_precondition_failed") from exc
    if not rows or rows[-1].get("event") != "attempt_reserved" or rows[-1].get("campaign_state") != "attempt_reserved":
        raise Gate5PaidPilotRetryCampaignV2Stop("no_incomplete_attempt_to_recover")
    sequence = rows[-1]["sequence"]
    if _completion_path(campaign_root, sequence).exists():
        raise Gate5PaidPilotRetryCampaignV2Stop("recovery_output_conflict")
    output = _output_path(campaign_root, sequence)
    try:
        completion, _, _ = _validate_attempt_output(output, components)
    except Exception as exc:
        raise Gate5PaidPilotRetryCampaignV2Stop("recovery_evidence_invalid") from exc
    if completion.get("sequence") != sequence or completion.get("output_directory_name") != rows[-1].get("output_directory_name"):
        raise Gate5PaidPilotRetryCampaignV2Stop("recovery_evidence_invalid")
...
def execute_once(credential_loader: Callable[[str], str], credential_target: str, transport: Any, attestation_path: Path, rate_path: Path, campaign_root: Path) -> dict[str, Any]:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV2Stop("pre_execution_validation_failed") from exc
    output = _output_path(campaign_root, sequence)
    try:
        ...
        if output.exists() and (output / "run_summary.json").exists():
            try:
                ...
                raise Gate5PaidPilotRetryCampaignV2Stop("campaign_incomplete_attempt") from exc
        else:
...
def main() -> int:
    ...
    if args.recover_incomplete:
        ...
        except Gate5PaidPilotRetryCampaignV2Stop as exc:
