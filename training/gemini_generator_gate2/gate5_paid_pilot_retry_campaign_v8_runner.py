"""Local-first V8 campaign with content-free schema evidence and reviewed resume."""
...
import gate5_paid_pilot_retry_campaign_v8_gate as campaign_gate
import gate5_paid_pilot_v8_engine as pilot
...
SCOPE_NOTE_PATH = PACKAGE / "gate5_paid_pilot_retry_campaign_v8_m02_start_diagnostic_proposal.md"
V7_DIRECTORY = PACKAGE / "gate5_paid_pilot_retry_campaign_v7_2026-08-17"
V7_ATTESTATION = PACKAGE / "gate5_paid_pilot_retry_campaign_v7_attestation_2026-08-17.json"
...
BUILD_PATHS = {"v8_proposal_sha256": SCOPE_NOTE_PATH}
CAMPAIGN_DIRECTORY_RE = re.compile(r"gate5_paid_pilot_retry_campaign_v8_(\d{4}-\d{2}-\d{2})(?:_r[2-9])?")

class Gate5PaidPilotRetryCampaignV8Stop(RuntimeError):
...
def canonical_hash(path: Path) -> str:
    ...
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV8Stop("canonical_input_invalid") from exc
...
def _validate_campaign_root(root: Path, execution_date: str) -> None:
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV8Stop("campaign_directory_invalid") from exc
    ...
    if resolved.parent != PACKAGE.resolve() or match is None or match.group(1) != execution_date:
        raise Gate5PaidPilotRetryCampaignV8Stop("campaign_directory_invalid")


def _new_file(path: Path, data: bytes) -> None:
    try:
        with path.open("xb") as handle: handle.write(data); handle.flush(); os.fsync(handle.fileno())
    except (FileExistsError, OSError) as exc: raise Gate5PaidPilotRetryCampaignV8Stop("exclusive_output_unavailable") from exc


def _append(path: Path, row: dict[str, Any]) -> None:
    if gate2.contains_secret(row): raise Gate5PaidPilotRetryCampaignV8Stop("secret_exposure")
    try:
        with path.open("ab") as handle: handle.write(gate2.canonical_json_bytes(row)); handle.flush(); os.fsync(handle.fileno())
    except OSError as exc: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_append_failed") from exc
...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid") from exc


def initial_components() -> list[dict[str, Any]]:
    digest = canonical_hash(V7_ATTESTATION)
    rows, components = v7.load_and_verify_campaign(V7_DIRECTORY, digest)
    context = pilot.validate_historical_components(components)
    expected = {"historical_pilot_component_count": 24, "historical_pilot_components_sha256": campaign_gate.INITIAL_COMPONENT_MANIFEST_SHA256, "historical_pilot_actual_usd_millionths": 234_960}
    if rows[-1].get("campaign_state") != "stopped_nonretryable_outcome" or rows[-1].get("attempts_reserved") != 6 or rows[-1].get("row_hash") != campaign_gate.EXPECTED_V7["v7_terminal_state_row_sha256"] or context != expected:
        raise Gate5PaidPilotRetryCampaignV8Stop("v7_terminal_state_invalid")
    terminal = components[-1]
    if terminal.get("row_hash") != campaign_gate.EXPECTED_V7["v7_terminal_component_row_sha256"] or terminal.get("terminal_disposition") != "proposed_output:output.bullets:02:protected_collision" or terminal.get("booked_cost_usd_millionths") != 10_680:
        raise Gate5PaidPilotRetryCampaignV8Stop("v7_terminal_component_invalid")
    return components


def verify_initial_evidence() -> dict[str, Any]:
    if canonical_hash(V7_ATTESTATION) != campaign_gate.EXPECTED_V7["v7_final_attestation_sha256"] or canonical_hash(V7_DIRECTORY / LEDGER_NAME) != campaign_gate.EXPECTED_V7["v7_terminal_campaign_state_sha256"]:
        raise Gate5PaidPilotRetryCampaignV8Stop("v7_terminal_evidence_invalid")
...
def verify_only() -> dict[str, Any]:
    ...
    for field, path in BUILD_PATHS.items():
        if campaign_gate.EXPECTED_BUILD[field] == "TO_BE_FILLED" or canonical_hash(path) != campaign_gate.EXPECTED_BUILD[field]:
            raise Gate5PaidPilotRetryCampaignV8Stop("v8_build_mismatch")
    ...
    if worst != 187_000 or context["historical_pilot_actual_usd_millionths"] + MAX_ATTEMPTS * worst != 3_039_960:
        raise Gate5PaidPilotRetryCampaignV8Stop("campaign_cost_math_invalid")
    build_hashes = campaign_gate.current_build_hashes()
    return {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v8_verify_only", **context, "maximum_campaign_attempts": MAX_ATTEMPTS, "worst_case_aggregate_usd_millionths": 3_039_960, **campaign_gate.EXPECTED_BUILD, **build_hashes, "network_used": False, "credential_read": False, "file_output_created": False}
...
def _state_row(event: str, sequence: int, state: str, attempts: int, context: dict[str, Any], lock_hash: str | None, completion_hash: str | None, output_name: str | None, pause_code: str | None, schema_hash: str | None, review_hash: str | None, rate_hash: str, rate_date: str, prior: str | None, attestation_hash: str) -> dict[str, Any]:
    row = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v8_state", "event": event, "sequence": sequence, "campaign_state": state, "attempts_reserved": attempts, **context, "attempt_lock_file_sha256": lock_hash, "completion_file_sha256": completion_hash, "output_directory_name": output_name, "pause_stop_code": pause_code, "schema_conformance_diagnostic_row_sha256": schema_hash, "review_artifact_sha256": review_hash, "effective_rate_snapshot_sha256": rate_hash, "effective_rate_date": rate_date, "prior_row_hash": prior, "campaign_proposal_sha256": campaign_gate.EXPECTED_BUILD["v8_proposal_sha256"], "attestation_sha256": attestation_hash}
...
def _load_output_rows(path: Path) -> list[dict[str, Any]]:
    try: return [json.loads(line, object_pairs_hook=campaign_gate.reject_duplicate_keys) for line in path.read_text(encoding="utf-8").splitlines() if line]
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV8Stop("attempt_output_invalid") from exc
...
def _validate_attempt_output(output: Path, components: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any], str]:
    try:
        ...
        if reservation.get("v8_schedule_sha256") != canonical_hash(pilot.SCHEDULE_PATH) or reservation.get("v8_first_request_sha256") != campaign_gate.EXPECTED_BUILD["v8_first_request_sha256"]:
            raise ValueError
        for index, receipt in enumerate(receipts):
            slot = slots[index]
            expected_request_hash = gate2.sha256_bytes(gate2.canonical_json_bytes(pilot.v7_prompt.build_request(slot)))
            if receipt.get("schedule_slot") != slot["campaign_schedule_sequence"] or receipt.get("model") != slot["model"] or receipt.get("mechanism_id") != slot["mechanism_id"] or receipt.get("prompt_hash") != slot["prompt_hash"] or receipt.get("request_hash") != expected_request_hash:
                raise ValueError
    ...
    elif summary.get("completed_slots") == 22 and stop is None: state, disposition = "stopped_completed", "full_pilot_completed"
...
def _zero_request_completion(output: Path, components: list[dict[str, Any]], sequence: int, stop: str) -> dict[str, Any]:
    if (output / "request_receipts.jsonl").exists() and (output / "request_receipts.jsonl").read_bytes(): raise Gate5PaidPilotRetryCampaignV8Stop("campaign_incomplete_attempt")
    names = ("pilot_reservation.json", "run_summary.json", "request_receipts.jsonl", "cost_ledger.jsonl", "rejection_ledger.jsonl", "output_collision_diagnostics.jsonl", "schema_conformance_diagnostics.jsonl", "candidate_quarantine.jsonl")
...
def _validate_state_row(row: Any, prior: str | None, attestation_hash: str) -> None:
    if not isinstance(row, dict) or set(row) != STATE_FIELDS or row.get("prior_row_hash") != prior or row.get("attestation_sha256") != attestation_hash or row.get("campaign_proposal_sha256") != campaign_gate.EXPECTED_BUILD["v8_proposal_sha256"]: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
    payload = {key: value for key, value in row.items() if key != "row_hash"}
    if row.get("row_hash") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)) or gate2.contains_secret(row): raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
...
def load_and_verify_campaign(root: Path, attestation_hash: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, row in enumerate(rows):
        ...
        if index == 0:
            ...
            if canonical_hash(_completion_path(root, row["sequence"])) != row["completion_file_sha256"] or completion.get("row_hash") != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)): raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
            ...
            if completion != derived or row["campaign_state"] != completion["campaign_state_after"] or row["pause_stop_code"] != completion["pause_stop_code"] or row["schema_conformance_diagnostic_row_sha256"] != completion["schema_conformance_diagnostic_row_sha256"]: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
            components.append(completion["component"])
        elif row["event"] == "pause_review_confirmed":
            if previous["campaign_state"] != "paused_pending_review" or row["campaign_state"] != "active_after_review" or row["sequence"] != previous["sequence"] or row["attempts_reserved"] != previous["attempts_reserved"] or not isinstance(row["review_artifact_sha256"], str): raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
            ...
            if review_lock.get("row_hash") != gate2.sha256_bytes(gate2.canonical_json_bytes(lock_payload)) or review_lock.get("paused_state_row_sha256") != previous["row_hash"] or review_lock.get("review_artifact_sha256") != row["review_artifact_sha256"]: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
            if canonical_hash(_review_path(root, row["sequence"])) != row["review_artifact_sha256"]: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
            review = gate2.load_json(_review_path(root, row["sequence"])); review_payload = {key: value for key, value in review.items() if key != "record_sha256"}; template = gate2.load_json(PACKAGE / "gate5_paid_pilot_retry_campaign_v8_review_template.json")
            if set(review) != set(template) or review.get("record_sha256") != gate2.sha256_bytes(gate2.canonical_json_bytes(review_payload)) or review.get("paused_campaign_state_row_sha256") != previous["row_hash"] or review.get("paused_attempt_sequence") != row["sequence"] or review.get("reviewed_by") != "Johnny" or review.get("pause_evidence_reviewed") is not True or review.get("resume_one_next_manual_attempt_authorized_by_johnny") is not True or review.get("effective_rate_snapshot_sha256") != row["effective_rate_snapshot_sha256"] or review.get("review_local_date") != row["effective_rate_date"] or review.get("effective_rate_tuple") != _rate_tuple(REFERENCE_RATE_PATH): raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
        else: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
        context = pilot.validate_historical_components(components)
        if any(row.get(key) != value for key, value in context.items()): raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
        prior = row["row_hash"]
...
def reserve_attempt(root: Path, attestation_hash: str, rate_path: Path) -> tuple[int, list[dict[str, Any]]]:
    ...
    if last["campaign_state"] in TERMINAL_STATES: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_already_terminal")
    if last["campaign_state"] == "paused_pending_review": raise Gate5PaidPilotRetryCampaignV8Stop("pause_review_required")
    if last["event"] == "attempt_reserved": raise Gate5PaidPilotRetryCampaignV8Stop("campaign_incomplete_attempt")
    if last["campaign_state"] not in ACTIVE_STATES: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_state_invalid")
    if local_today() != last["effective_rate_date"] or canonical_hash(rate_path) != last["effective_rate_snapshot_sha256"]: raise Gate5PaidPilotRetryCampaignV8Stop("execution_day_rate_snapshot_mismatch")
    if _rate_tuple(rate_path) != _rate_tuple(REFERENCE_RATE_PATH): raise Gate5PaidPilotRetryCampaignV8Stop("execution_day_rate_snapshot_mismatch")
    sequence = last["attempts_reserved"] + 1
    if sequence > MAX_ATTEMPTS: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_attempt_cap_reached")
    context = pilot.validate_historical_components(components); output_name = _output_path(root, sequence).name
    lock = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v8_attempt_lock", "sequence": sequence, "created_utc": utc_now(), "prior_state_row_hash": last["row_hash"], **context, "attestation_sha256": attestation_hash, "output_directory_name": output_name, "effective_rate_snapshot_sha256": last["effective_rate_snapshot_sha256"], "effective_rate_date": last["effective_rate_date"], "state": "reserved_before_credential_read"}
...
def _validate_review_artifact(path: Path, root: Path, attestation_path: Path, fresh_rate_path: Path | None) -> tuple[dict[str, Any], str, str]:
    try:
        value = json.loads(path.read_text(encoding="utf-8"), object_pairs_hook=campaign_gate.reject_duplicate_keys); template = gate2.load_json(PACKAGE / "gate5_paid_pilot_retry_campaign_v8_review_template.json")
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV8Stop("pause_review_invalid") from exc
    if not isinstance(value, dict) or set(value) != set(template): raise Gate5PaidPilotRetryCampaignV8Stop("pause_review_invalid")
    supplied_hash = value.get("record_sha256"); payload = {key: item for key, item in value.items() if key != "record_sha256"}
    if supplied_hash != gate2.sha256_bytes(gate2.canonical_json_bytes(payload)) or gate2.contains_secret(value): raise Gate5PaidPilotRetryCampaignV8Stop("pause_review_invalid")
    ...
    fixed = {
        "v8_final_attestation_sha256": attestation_hash, "v8_proposal_sha256": campaign_gate.EXPECTED_BUILD["v8_proposal_sha256"], "paused_campaign_state_row_sha256": paused["row_hash"], "paused_attempt_sequence": paused["sequence"], "paused_attempt_lock_sha256": paused["attempt_lock_file_sha256"], "paused_attempt_completion_sha256": paused["completion_file_sha256"], "paused_output_evidence_sha256": completion["output_evidence_sha256"], "pause_stop_code": paused["pause_stop_code"], "schema_conformance_diagnostic_row_sha256": paused["schema_conformance_diagnostic_row_sha256"], "current_attempts_reserved": paused["attempts_reserved"], "current_attempts_remaining": MAX_ATTEMPTS - paused["attempts_reserved"], "current_historical_component_count": context["historical_pilot_component_count"], "current_historical_components_sha256": context["historical_pilot_components_sha256"], "current_historical_actual_usd_millionths": context["historical_pilot_actual_usd_millionths"], "maximum_campaign_attempts": MAX_ATTEMPTS, "reconciliation_stop_usd_millionths": pilot.V8_RECONCILIATION_STOP, "pilot_ceiling_usd_millionths": pilot.V8_PILOT_CEILING, "prior_rate_snapshot_sha256": paused["effective_rate_snapshot_sha256"], "pause_local_date": paused["effective_rate_date"],
    }
    if any(value.get(key) != item for key, item in fixed.items()): raise Gate5PaidPilotRetryCampaignV8Stop("pause_review_invalid")
    mode = value.get("review_mode"); today = local_today()
    if value.get("review_local_date") != today or mode not in {"same_day", "next_day"}: raise Gate5PaidPilotRetryCampaignV8Stop("pause_review_invalid")
    prior_tuple = _rate_tuple(REFERENCE_RATE_PATH)
    if value.get("prior_rate_tuple") != prior_tuple: raise Gate5PaidPilotRetryCampaignV8Stop("pause_review_invalid")
...
def confirm_pause_review(review_path: Path, attestation_path: Path, campaign_root: Path, fresh_rate_path: Path | None = None) -> dict[str, Any]:
    ...
    review_lock = {"artifact": "gemini_generator_gate5_paid_pilot_retry_campaign_v8_pause_review_lock", "sequence": paused["sequence"], "paused_state_row_sha256": paused["row_hash"], "review_artifact_sha256": review_hash, "created_utc": utc_now()}
...
def _pilot_attestation_view(path: Path, execution_date: str) -> dict[str, Any]:
    """Validate V8, then expose the already-reviewed legacy engine fields."""
...
def execute_once(credential_loader: Callable[[str], str], credential_target: str, transport: Any, attestation_path: Path, rate_path: Path, campaign_root: Path) -> dict[str, Any]:
    try:
        ...
        if isinstance(exc, Gate5PaidPilotRetryCampaignV8Stop): raise
        raise Gate5PaidPilotRetryCampaignV8Stop("pre_execution_validation_failed") from exc
    ...
    try:
        ...
        if output.exists() and (output / "run_summary.json").exists():
            ...
            except Exception: raise Gate5PaidPilotRetryCampaignV8Stop("campaign_incomplete_attempt") from exc
...
def recover_incomplete_attempt(attestation_path: Path, campaign_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise Gate5PaidPilotRetryCampaignV8Stop("recovery_precondition_failed") from exc
    last = rows[-1]
    if last.get("event") != "attempt_reserved" or last.get("campaign_state") != "attempt_reserved": raise Gate5PaidPilotRetryCampaignV8Stop("no_incomplete_attempt_to_recover")
    sequence = last["sequence"]
    if _completion_path(campaign_root, sequence).exists(): raise Gate5PaidPilotRetryCampaignV8Stop("recovery_output_conflict")
    try: completion = _validate_attempt_output(_output_path(campaign_root, sequence), components)[0]
    except Exception as exc: raise Gate5PaidPilotRetryCampaignV8Stop("recovery_evidence_invalid") from exc
    if completion.get("sequence") != sequence or completion.get("output_directory_name") != last.get("output_directory_name"): raise Gate5PaidPilotRetryCampaignV8Stop("recovery_evidence_invalid")
    return complete_attempt(campaign_root, sequence, components, attestation_hash, completion)
...
def main() -> int:
    ...
    except Gate5PaidPilotRetryCampaignV8Stop as exc: print(json.dumps({"disposition": "stopped", "stop_reason": exc.code}, sort_keys=True)); return 2
