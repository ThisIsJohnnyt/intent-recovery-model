...
import gate5_paid_pilot_retry_campaign_v12_gate as gate
import gate5_paid_pilot_retry_campaign_v12_runner as runner
import gate5_paid_pilot_v12_engine as engine
...
class Gate5PaidPilotRetryCampaignV12Tests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.v11_components = runner.v11_terminal_components()
    ...
    def attestation_stub(self, root: Path, rate: Path, execution_date: str = FICTIONAL_DATE) -> tuple[Path, dict]:
        value = gate2.load_json(PACKAGE / "gate5_paid_pilot_retry_campaign_v12_attestation_template.json")
        value.update({**gate.EXPECTED_V11, **gate.EXPECTED_BUILD, **gate.current_build_hashes()})
        ...
        value["pilot_ceiling_usd_millionths"] = engine.V12_PILOT_CEILING
        value["reconciliation_stop_usd_millionths"] = engine.V12_RECONCILIATION_STOP
        ...
        value["v12_worst_case_aggregate_usd_millionths"] = 1_989_949
    ...
    def _fresh_named_campaign_dir(self) -> Path:
        path = PACKAGE / f"gate5_paid_pilot_retry_campaign_v12_{self.FICTIONAL_DATE}"
    ...
    def test_v11_terminal_evidence_rederived_and_pinned(self) -> None:
        context = engine.validate_historical_components(self.v11_components)
        self.assertEqual(context, {
            "historical_pilot_component_count": 28,
            "historical_pilot_components_sha256": gate.INITIAL_COMPONENT_MANIFEST_SHA256,
            "historical_pilot_actual_usd_millionths": 1_054_949,
        })

    def test_v12_reuses_v8_schedule_unchanged(self) -> None:
        ...
        self.assertEqual(first_hash, engine.EXPECTED_V12_FIRST_REQUEST_SHA256)
    ...
    def test_cost_math_and_verify_only_have_no_side_effects(self) -> None:
        ...
        self.assertEqual(value["historical_pilot_actual_usd_millionths"], 1_054_949)
        self.assertEqual(value["single_pass_reservation_usd_millionths"], 187_000)
        self.assertEqual(value["worst_case_aggregate_usd_millionths"], 1_989_949)
    ...
    def test_engine_has_no_standalone_credential_entrypoint(self) -> None:
        source = (PACKAGE / "gate5_paid_pilot_v12_engine.py").read_text(encoding="utf-8")
        self.assertNotIn("--execute-full-schedule", source)
        self.assertNotIn("gate4_connectivity_runner", source)
        self.assertIn("dedicated V12 campaign runner", source)
    ...
    def test_attestation_template_remains_execution_disabled(self) -> None:
        ...
        self.assertFalse(template["v12_campaign_execution_authorized_by_johnny"])
        self.assertEqual(gate.EXPECTED_BUILD["v12_first_request_sha256"], engine.EXPECTED_V12_FIRST_REQUEST_SHA256)

    def test_campaign_directory_naming(self) -> None:
        self.assertIsNotNone(runner.CAMPAIGN_DIRECTORY_RE.fullmatch("gate5_paid_pilot_retry_campaign_v12_2026-08-18"))
        self.assertIsNone(runner.CAMPAIGN_DIRECTORY_RE.fullmatch("gate5_paid_pilot_retry_campaign_v11_2026-08-18"))
        self.assertIsNone(runner.CAMPAIGN_DIRECTORY_RE.fullmatch("gate5_paid_pilot_retry_campaign_v12_2026-08-18_extra"))

    # -- the stopword-filtered char-5-gram fix itself, in isolation -------

    def test_stopword_only_overlap_no_longer_collides_on_char5gram_either(self) -> None:
        """Direct proof the fix does what it claims: the exact pair V11's
        test suite used to prove token-Jaccard was fixed (and which V11
        deliberately left colliding on char-5-gram, since only token-Jaccard
        was in scope then) now also drops to 0.0 char-5-gram, and token-
        Jaccard is unaffected by this build (still 0.0, unchanged from V11)."""
        ...
        v11_scoring = gate2.collision_check_stopword_filtered(candidate, [("ref", reference)])
        v12_scoring = gate2.collision_check_fully_stopword_filtered(candidate, [("ref", reference)])
        self.assertGreater(v11_scoring["maximum_character_5gram_jaccard"]["score"], gate2.CHAR_5GRAM_JACCARD_THRESHOLD)
        self.assertTrue(v11_scoring["fatal"])
        self.assertEqual(v12_scoring["maximum_character_5gram_jaccard"]["score"], 0.0)
        self.assertEqual(v12_scoring["maximum_token_jaccard"]["score"], v11_scoring["maximum_token_jaccard"]["score"])
        self.assertFalse(v12_scoring["fatal"])

    def test_genuine_content_overlap_still_collides_under_char5gram_filtering(self) -> None:
        ...
        new = gate2.collision_check_fully_stopword_filtered(candidate, [("ref", reference)])
        self.assertTrue(new["fatal"])
        self.assertGreaterEqual(new["maximum_token_jaccard"]["score"], gate2.TOKEN_JACCARD_THRESHOLD)
        self.assertGreaterEqual(new["maximum_character_5gram_jaccard"]["score"], gate2.CHAR_5GRAM_JACCARD_THRESHOLD)

    def test_core_new_v12_behavior_real_corpus_flip_from_rejected_to_accepted(self) -> None:
        """The core new V12 behavior, exercised directly through the real
        111-record corpus: a short, stopword-dense candidate that scores
        below V11's token-Jaccard threshold but above the unchanged
        char-5-gram threshold -- a real collision under V11's scoring and a
        real, clean accept under V12's -- mirroring the exact real pattern
        V11's production run showed (one candidate matching 28 different
        references, all char-5-gram driven, at 0.102-0.109)."""
        candidate = {
            "source_input": " ".join(f"zqfwordinput{i:03d}" for i in range(80)),
            "proposed_output": {
                "narrative": "It was noted that the plan is to review this on the following day for the team.",
                "bullets": [
                    "It was noted that the plan is to review this with the group on the following week.",
                    "This is the case that the item was set for the review on the following day for the team.",
                ],
                "action_items": ["Follow up with the group on this for the team on the following day."],
            },
        }
        old = gate2.screen_candidate_stopword_filtered(candidate, self.references, [], [])
        new = gate2.screen_candidate_fully_stopword_filtered(candidate, self.references, [], [])
        self.assertTrue(old["fatal"], "fixture must be a real collision under V11's scoring")
        self.assertFalse(new["fatal"], new.get("fatal_reasons"))

    def test_short_string_char5gram_stopword_removal_can_raise_not_only_lower_score(self) -> None:
        """Carried forward from V11's identical finding on the token side:
        for very short strings, stopword removal does not only ever lower
        Jaccard similarity -- the same union-shrinkage property applies to
        character-5-grams too. Kept here as documented, intentional
        behavior rather than silently avoided."""
        candidate = "on it."
        reference = "the plan is on it now for the team."
        old = gate2.char_5grams(candidate)
        new = gate2.char_5grams_stopword_filtered(candidate)
        old_ref = gate2.char_5grams(reference)
        new_ref = gate2.char_5grams_stopword_filtered(reference)
        old_score = gate2.jaccard(old, old_ref)
        new_score = gate2.jaccard(new, new_ref)
        # Not asserting a specific direction here beyond what's documented:
        # just that the two scores can genuinely differ for short strings,
        # confirming the mechanism is real and exercised, not theoretical.
    ...
    def test_continue_past_collision_and_schema_invalid_but_not_secret_exposure(self) -> None:
        """Same continue-past ruleset as V11, unchanged: a collision and a
        genuine schema_invalid both continue the run; secret_exposure still
        hard-stops it immediately."""
        accept_body = self.make_body()  # still a real collision under V12's scoring too
        ...
        with patch.object(engine, "load_schedule", return_value=four_slots):
            with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
                ...
                summary = engine.execute_pilot(
                    ...
                    historical_components=self.v11_components,
    ...
    def test_two_schema_invalid_cards_in_one_run_does_not_crash(self) -> None:
        """Regression guard carried forward from V10/V11: gate5_schema_
    ...
    def test_real_v12_runner_clean_503_end_to_end(self) -> None:
        clean_second_card = {
            "source_input": " ".join(f"zqvornex{i}" for i in range(80)),
            # action_items intentionally longer/more distinctive, same
            # lesson learned from V11's own equivalent fixture: a short
        ...
        # "unexpected_local_error" -- same documented pattern used in V9's,
        # V10's, and V11's equivalent tests.
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            root = Path(temp)
            ...
            events: list[dict] = []
            with patch("gate5_paid_pilot_retry_campaign_v12_gate.date", self._FrozenFictionalDate), patch.object(runner, "local_today", return_value=self.FICTIONAL_DATE):
    ...
    def test_credential_failure_is_zero_cost_and_no_network(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE) as temp:
            ...
            campaign = self._fresh_named_campaign_dir()
            with patch("gate5_paid_pilot_retry_campaign_v12_gate.date", self._FrozenFictionalDate), patch.object(runner, "local_today", return_value=self.FICTIONAL_DATE):
                with self.assertRaises(runner.Gate5PaidPilotRetryCampaignV12Stop):
