"""Standalone assertion tests for run_seed17_contrastive_replay.py -- dummy
...
directly with `python test_run_seed17_contrastive_replay.py`, matching
this repo's existing script-based tooling convention.
...
Covers everything specific to this dual-arm package on top of the prior
Phase-2 replay wrapper's already-proven design: canonicalize_pinned_lf_bytes
(ported, shared between nine governing inputs); load_canonical_governing_inputs;
ensure_real_validation_placeholder (create-if-missing, never overwrite);
...
real scratch git repositories (gitignore-protected, unprotected, and an
unrelated-dirty-tree case) proving the corrected verify-before-mutate
ordering, not the gitignore entry, is what makes the sequence safe; added
resolve_experiment_dir tests (relative path resolves against TRAINING_DIR,
...
import run_seed17_contrastive_replay as w
...
# ---------------------------------------------------------------------------
# canonicalize_pinned_lf_bytes -- ported from the corpus-derivation script;
# same coverage as that script's own test suite, since this is a separate
# copy (not an import) and must be independently proven correct here too.
# ---------------------------------------------------------------------------
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    try:
        ...
        # Idempotent / non-destructive: running again with the file already
        # present (empty) must not error and must not change it.
        w.ensure_real_validation_placeholder()
        check("placeholder step is a no-op when the file already exists (empty)", rv_path.stat().st_size == 0)

        # Never overwrites -- even a non-empty existing file is left alone;
        # verify_real_validation_empty() is the one place that fails
        # closed on that, not this creation step.
...
# Added 2026-08-10 per ChatGPT's second review: reported that placeholder
# creation before clean-tree verification would make a fresh checkout's own
# `git status` report the new file as untracked, failing preflight. Claude
# independently checked this and found it does not currently reproduce --
# datasets/.gitignore already covers this path and is itself committed at
# the pinned milestone. These three scenarios prove: (1) the corrected
# sequence succeeds when the path IS gitignore-protected (matching the real
# repository); (2) it ALSO succeeds when the path is NOT protected --
# proving the ordering itself, not the gitignore entry, is what makes this
# safe; (3) dirty-tree rejection for an unrelated cause is not weakened by
# the reordering, and the placeholder is never created when that check
# fails first.
# ---------------------------------------------------------------------------
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    try:
        ...
        check(
            ...
            "ordering itself (verify captured state before mutating), not the gitignore entry, is "
...
# ---------------------------------------------------------------------------
# resolve_experiment_dir -- added 2026-08-10 per ChatGPT's second review: a
# relative --experiment-dir reproduced an uncontrolled ValueError.
# ---------------------------------------------------------------------------

print("\n=== resolve_experiment_dir ===")

check("default (None) resolves under TRAINING_DIR", w.resolve_experiment_dir(None) == w.TRAINING_DIR / "controlled_seed17_contrastive_replay_run")
...
# The actual reported reproduction: build_commands() must not raise a raw
# ValueError for a resolved relative experiment dir.
...
try:
    ...
    check("build_commands() no longer raises for a resolved relative --experiment-dir", True)
except ValueError as e:
    check("build_commands() no longer raises for a resolved relative --experiment-dir", False, detail=str(e))
...
treatment_candidate_bytes = make_pair_bytes(82, "cand")
treatment_train_bytes = make_pair_bytes(76, "train")
...
msg = expect_system_exit(
    ...
    82, 76, 6, synthetic_treatment_fp, "synthetic-treatment",
)
check("correct 82/76/6 split with matching fingerprint passes", msg is None, detail=str(msg))

msg = expect_system_exit(
    w.verify_arm_split_and_fingerprint, synthetic_canonical, "candidate.jsonl", "train.jsonl", "val.jsonl",
    82, 76, 6, "0" * 64, "synthetic-treatment",
)
check("training-data fingerprint mismatch fails closed", msg is not None)

msg = expect_system_exit(
    w.verify_arm_split_and_fingerprint, synthetic_canonical, "candidate.jsonl", "train.jsonl", "val.jsonl",
    82, 75, 6, synthetic_treatment_fp, "synthetic-treatment",
)
check("wrong expected train count fails closed", msg is not None)

msg = expect_system_exit(
    w.verify_arm_split_and_fingerprint, synthetic_canonical, "candidate.jsonl", "train.jsonl", "val.jsonl",
    81, 76, 6, synthetic_treatment_fp, "synthetic-treatment",
)
...
check("all proposal records in train, none in val passes", msg is None, detail=str(msg))
...
check("a proposal record present in val fails closed", msg is not None)
...
dup_ids_canonical = {
    w.PROTECTED_BENCHMARK_REL: make_bench_bytes(16, "p"),
    w.ACCEPTANCE_BENCHMARK_REL: make_bench_bytes(10, "p"),  # colliding prefix -> duplicate IDs across the two files
}
...
# ---------------------------------------------------------------------------
# build_commands / verify_command_shape -- both arms symmetric this time
# ---------------------------------------------------------------------------
...
check("treatment_train resolves to the contrastive data dir", w.TREATMENT_DATA_DIR_REL in commands["treatment_train"])
...
# Cross-arm confusion: treatment command pointed at the comparator's data dir.
...
# Cross-check against a real, previously-generated raw artifact from the
# prior Phase-2 replay (same 16-probe protected benchmark content, unscored
# scaffold shape) -- proves this adapted function still accepts genuine
# real-world output, not just synthetic fixtures.
...
if real_protected_path.exists():
    ...
    check("a real, genuinely-unscored protected-16 artifact from the prior wrapper still passes", msg is None, detail=str(msg))
...
try:
    result = subprocess.run(
        [sys.executable, str(w.TRAINING_DIR / "run_seed17_contrastive_replay.py"), "--experiment-dir", str(fresh_experiment_dir)],
        cwd=w.TRAINING_DIR,
        capture_output=True,
        text=True,
    )
...
check(
    "gate-6 pass set matches the Phase-2 replay's pass set exactly (unchanged)",
...
if isinstance(real_canonical_inputs, dict):
    ...
    check("real treatment split (82/76/6) and fingerprint verify clean", msg is None, detail=str(msg))
    ...
    check("real treatment proposal membership (16 records) verifies clean", msg is None, detail=str(msg))

real_commands = w.build_commands(
    w.TRAINING_DIR / "controlled_seed17_contrastive_replay_run" / "treatment",
    w.TRAINING_DIR / "controlled_seed17_contrastive_replay_run" / "comparator",
)
